...
而所有收获均不依赖于特定语言、框架和方法。 时隔 20 年的新版，
经过全面的重新选材、组织和编写，
覆盖哲学、方法、工具、设计、解耦、并发、重构、需求、
团队等务实话题的最佳实践及重大陷阱，
以及易于改造、复用的架构技术。 本书极具洞察力与趣味性，
适合从初学者到架构师的各层次读者潜心研读或增广见闻。
...
我还记得 Dave 和 Andy 第一次在推特上谈论这本书新版的那一刻——这可是一条大新闻。
在编程社区， 到处都是对这条大新闻的兴奋回应， 人们的期待塞满了我的信息流。
二十年过去了， 《程序员修炼之道》这本书的地位不逊于当年。

一本承载了这样一段历史的书， 能引起这样的反响， 本身就说明了很多问题。
...
上面的灰尘已被掸去， 并且涂上了一层新油漆——这确实让它熠熠生辉。
...
因为它理解身为一名程序员到底意味着什么。 编程涉及方方面面——尽量减少未来的痛苦，
让队友更轻松， 做错事情后能够重新振作起来， 养成良好的习惯， 以及理解工具集。
编程只是程序员世界的一部分， 而这本书探索了整个世界。

我曾花很多时间思考编码之旅。 我不是从小就开始接触编程的，
...
这对我来说， 真像进入一个全新的世界。 就像搬到一个新的城镇，
也有必要了解邻居、挑选杂货店、找到最好的咖啡店。 我花了一段时间来了解地形，
找到了最有效的路线， 避开了交通最繁忙的街道，
并且知道了什么时候交通可能会出问题。 等到天气变化， 我还得去置办应季的新衣。

来到一个新城镇的头几周， 甚至是头几个月， 可能会很害怕。
如果有一个已经在这里住了一段时间的邻居， 而且他知识渊博又友好，
那不是再好不过的事情吗？ 谁能带你四处参观， 谁能领你去那些咖啡店？
当然是一个在当地待得足够久、 了解当地文化和脉搏的人。
这样你不仅有家的感觉， 自己也能成为有贡献的一员。 Dave 和 Andy
就是这样的邻居。

一个准新人， 更容易对成为程序员的过程， 而不是对编程行为本身不知所措。 因此，
必须对整个心态做一次切换——改变习惯、行为和期望。 仅仅知道如何编程，
并不会让你成为一名更好的程序员， 这个过程需要刻意而有意识的练习。
...
还是充分利用这多出来的二十年经验， 重新审视前一版所推崇的实践背后的种种假设。
...
因为当前有着大量的并行硬件， 却缺乏处理并行的好方法。
我们还添加了一些内容来反映不断变化的认知和环境， 从我们帮助发起的敏捷运动，
到对函数式编程日益广泛的接受， 再到对隐私和安全方面日益增长的需求。

然而有趣的是， 我们之间关于书中内容的争论比写第一版时要少得多。
重要的东西更容易辨别， 这已是我们的共识。
...
你可以按任意次序随意阅读这些主题——这不是一本需要从头到尾阅读的书。
...
> “我用一个词，”矮胖子相当傲慢地说，“总是同我想要说的恰如其分的，既不重，
> 也不轻。”
>
> ——路易斯·卡罗《爱丽丝镜中奇遇》
...
而且像对象和关系型数据库这种已被广泛使用的， 再下定义就有点画蛇添足了。
...
最后，
向所有在过去的二十年里一直致力于让编程变得更好的务实的程序员们发出一声呐喊：
再接再厉，
更创二十年辉煌。
...
调整方法去适应当前的情况和环境。 对影响项目的各项因素的相对重要性做出判断，
并通过经验找到适当的解决方案。 随着工作的进展， 你要不断地这样做。
务实的程序员不仅把工作做完， 而且做得很好。

### 谁应该读这本书

本书的目标读者是那些希望成为更高效、多产程序员的人。 也许你现在感到沮丧，
...
每个开发人员都是独特的， 有各自的优势和劣势， 以及各自的偏好和厌恶。
随着时间的推移， 每个人都将打造自己的个人环境。
这种环境将像程序员的爱好、衣服或发型一样， 能强烈地反映出他或她的个性。 然而，
作为务实的程序员， 你们会共有如下许多特征：
...
你倾向于问问题。 这真不错——你怎么做到的？ 你对那个库有什么看法？
...
你试图理解所面临的每个问题的本质。
这种现实主义让你对事情有多困难、需要用多长时间有很好的把握。
深刻理解哪些过程本来就该很难、 需要点时间才能完成，
会给你坚持下去的毅力。
...
> 这不是对当前实践做的一次性审计
> ——而是对每天里每一个项目所做的每一个决定进行的批判性评估。
> 不要用自动辅助驾驶系统偷懒， 而是要不断地思考， 实时地批判自己的工作。 IBM
...
“软件是一门工程学科，”他们说， “如果团队成员各行其是， 软件就会崩溃。”
...
> 图 0-1：读者服务微信二维码。
> 微信扫码回复：38435，可获取本书配套代码资源、加入读者交流群、
> 获取博文视点学院付费内容抵扣券、技术专家分享视频与学习资源，
> 以及本书提及所有网址的“链接列表”。
...
若不了解来龙去脉， 又何谈结合实际？ 又怎能做出明智的妥协和合理的决策？

他们的另一个特质， 是对所做的一切负责。 关于这一点，
我们会在[我的源码被猫吃了](#2-我的源码被猫吃了)中讨论。
出于责任感， 务实的程序员不会在自己的项目分崩离析时坐视不管。
...
一旦这些事情发生， 就要依靠我们的专业性去解决问题。 这意味着要诚实和坦率。
...
> ）。
> 为了防止错误再次发生，
> 你是否需要引入更好的测试（参见[话题 41：为编码测试](#41-为编码测试)
> 和无情的持续测试）或增加自动化流程？
>
...
- 当你意识到自己在说“我不知道”时， 一定要接着说“——但是我会去搞清楚”。
  用这样的方式来表达你不知道是非常好的，
  因为接着你就可以像专业人士一样承担起责任。

### 3 软件的熵

虽然软件开发不受绝大多数物理法则的约束， 但我们无法躲避熵增带来的重击。
...
有很多因素会导致软件腐烂。 最重要的一个似乎是项目工作中的心理状态，
或者说文化。 即使是一个单人团队， 你项目的心理状态也是非常脆弱的东西。
...
只需一样东西就能非常迅速地把一幢干净完好的宜居建筑变成破败的废楼。[^ch02-3]

一扇破窗。

一扇破损的窗户， 只要一段时间不去修理，
建筑中的居民就会潜移默化地产生一种被遗弃感——觉得管理方并不关心这幢建筑。
...
就像密闭空间里的流感病毒。 无视一个明显损坏的东西， 会强化这样一种观念：
...
现在我们知道， 一旦窗户开始破损， 运转良好的干净系统就会迅速恶化。
...
出于对自己控制这场火势能力的绝对自信， 还是尽力兼顾了不对财物造成不必要的损害。
这也是软件开发中应该遵循的方法： 不要只是因为一些东西非常危急，
就去造成附带损害。 破窗一扇都嫌太多。

一扇破窗——一段设计糟糕的代码，
一个让团队在整个项目周期内都必须遵守的糟糕管理决定——就是一切衰退的开始。
如果你发现自己正处在有几扇破窗的项目中，
就非常容易陷入这样的想法——“反正代码所有其他部分都是一坨屎， 我只是随大流而已。”
项目在这之前是否一直运转良好并不重要。 在最初启发“破窗理论”的实验中，
...
- 通过在项目的社区中做调研来帮助团队变强。 选两三扇破窗，
  和同事讨论问题出在哪里， 以及可以做些什么来修复。
- 你能说出第一扇破窗是何时产生的吗？ 你对此有何反应？ 如果这是别人做出的决定，
  或者来自管理层的命令， 那你能做点什么吗？

### 4 石头做的汤和煮熟的青蛙

三个从战场归来的士兵饥肠辘辘。 他们看到前方有一座村庄，
顿时重整精神——他们觉得村民们会给口饭吃。 可是当他们抵达那里时，
却发现四处门窗紧闭。 连年战乱之下， 村民们食物短缺， 仅有的存粮都藏了起来。
...
说这样汤会更鲜美。 每次都有不同的村民跑回去取来自己的私藏。

最后， 他们煮了一大锅热气腾腾的汤。 士兵们把汤里的石头扔掉，
和全村村民一起分享了一顿美餐， 这是他们所有人几个月以来吃的第一顿饱餐。
...
订单完成， 货也送到了。 只是在那一大箱芯片之外， 还有一个小盒子， 装着十片芯片，
盒子上贴着的标签写着“这些是次品”。
...
只要好到能让你自己内心平静就行。 你会发现， 你变得更有效率，
用户也更快乐。 而且， 可能让你更开心的是，
更短的孵化期促使你的程序实际上更好了。

在进一步讨论之前， 我们需要对将要讨论的内容做一些限定。
“够好即可”这个词并不意味着草率或糟糕的代码。
所有系统必须满足用户的需求才算完成， 并达到基本的性能、隐私和安全标准。
你做的东西， 从用户需求角度来说是否足够好？ 最好还是留给用户一个机会，
让他们能亲自参与评判。

#### 让用户参与权衡

通常你是为别人开发软件， 要记得弄清他们的需求。[^ch02-7]
...
章[当你在编码时](#第-7-章-当你编码时)
中我们将讨论在不完美的世界中开发代码的哲学。）
...
鉴于技术圈变化的速度越来越快， 这种事情可能会发生得特别迅速。

当你的知识价值下降时， 你对于公司或客户的价值也在下降。 我们想阻止这一切的发生。

学习新事物的能力是你最重要的战略资产。 但是该如何获得这种能力，
又如何知道该学什么呢？
...
也许你需要温习一下有段时间没用过的数据库技术。 或者， 不妨去尝试另一种语言，
它可能会让你在新的工作中处于更有利的地位……

在所有这些指导方针中， 下面这条最重要， 做起来反而最简单。
...
不要忘记其中人的因素， 它需要完全不同的技能集（我们称这些为软技能，
...
可以了解那些使用不同技术的人的经验， 以及他们所用的特殊术语， 等等。

持续投资非常重要。 一旦你对某门新语言或新技术进入舒适区， 向前走， 再学一个。

你是否在项目中使用过这些技术并不重要， 甚至要不要把它们放在你的简历中也不重要。
学习的过程将会扩展你的思维， 为你打开全新可能性的大门， 让你领悟新的做事方式。
想法之间的交叉借鉴很重要； 试着把你领悟到的东西应用到你当前的项目中。
...
尽管如此， 当有人问你问题时， 如果确实毫无思路， 也只能坦率地承认自己无法作答。

但不要停在这里， 把找到答案作为一项个人挑战。 问问周围的人，
或是上网搜索——不要仅限于大众领域， 还要去学术领域找一找。
...
并不能说明这是你的最佳选择， 而且内容提供商也可以花钱把自己的内容排到前列。
...
- 本周就开始学习一门新语言。 你是不是一直在用同一门古老的语言编程？ 试试
  Clojure、Elixir、Elm、F#、Go、Haskell、Python、R、ReasonML、Ruby、Rust、
Scala、Swift、Typescript，
  或是其他你看过去感觉会喜欢的语言。[^ch02-11]
- 开始读一本新的书（不过一定要先把我们这本书读完！）。
  如果你正在做非常细致的实现和编码工作， 就去读一本讲设计和架构的书。
...
假设你想提出一个建议： 用一个基于网页的系统让最终用户提交错误报告。
 根据听众的不同，
...
与所有的沟通形式一样， 这里的窍门是收集反馈。 不要只是等着别人提问：
把它问出来。 注意看肢体语言和面部表情。 神经语言程序学有一个前提假设，
“沟通的意义在于你得到的回应。” 在交流的过程中， 不断提高对听众的了解。
...
是专家还是新手？ 他们需要手把手教， 还是只想要一个“太长不看”版的简介？
如果有疑虑， 开口问。

不过， 还是要记住， 相互沟通这件事你占了其中的一半。
如果有人想让你用一段话谈谈某件事， 而你发现那不是三两句就讲得清楚的，
那么就如实告知。 记住， 这样的反馈也是一种交流方式。
...
充其量视其为一项不得已而为之的任务，
最糟糕的情况是把它当成一个低优先级任务——最好项目经理在项目结束时能把它忘掉。
...
但是为了方便起见， 我们把这些想法和过程集中在这里。

首先， 是触及软件开发核心的至关重要的主题：优秀设计的精髓。
所有后面的内容都来源于此。
...
- 再思考一下语言和编程范式（面向对象、函数式编程、响应式编程，等等）。
  当利用它们写出
  ETC
  的代码时，有什么地方起了很大的正面作用或负面效果？
在编码时你能做点什么来避免负面效果，或加强正面效益？
[^ch03-ch2-3]
- 很多编辑器支持（不论是内建支持还是通过扩展支持）在你保存文件时运行一些指令。
  可以试着让你的编辑器在每次存盘的时候弹出一个
...
作为程序员， 我们做的就是收集、组织、维护以及归类知识。 我们把知识文档化，
...
这的确是 DRY 的组成部分， 但只是一个很小的部分， 也绝非最重要的部分。
...
或是同时变更数据库 Schema 和代码中相关的数据结构， 抑或……？ 如果这类情况发生，
你的代码并不满足 DRY。
...
准确的命名即可表述清楚要做什么。 如果有人需要了解更多细节， 源码里应有尽有。
这就是 DRY！
...
这里的技巧可以将负面影响限制在局部。
违背的部分不会被暴露到外部世界：只有类里面的方法才需要担心相关行为的正确性。
...
就意味着， 所有使用这个数据结构的代码都与模块的实现产生了耦合。 但凡有可能，
...
DRY：代码必须持有外部实体已经蕴含的知识。 它需要了解 API、数据的
...
最难检测到且最难处理的重复类型， 可能发生在同一项目的不同开发人员之间。
整块的功能可能在不经意间被重复实现， 而这种重复或许好几年都未被发现，
最终导致维护问题。 这里有一个例子，
是我们从当事人那里直接听来的：
当调查美国一个州的政府电脑系统是否已处理好千年虫问题时，
审计找出了超过 10,000 个程序， 每个程序都包含有不同版本的社保号验证代码。

从较高的层面讲， 应建立一个强大的、紧密联系的、沟通良好的团队来解决这个问题。
...
可以每天来一次 Scrum 晨会。 可以开一个论坛（比如 Slack 频道）用于讨论常见问题。
这提供了一种非侵入性的通信方式——甚至可以跨越多个场所
——为所说的每件事都留下永久记录。
...
数据库相关代码应该和用户界面保持正交：你可以变更界面而不影响数据库，
切换数据库而不必更换界面。
...
推动它， 直升机就会向相应的方向移动； 你的左手握着的是总距操纵杆，
...
只要编写正交的系统， 就能获得两个主要的收益：提高生产力及降低风险。

#### 获得生产力

- 将变更限制在局部后，开发时间和测试时间都会减少。
  编写相对较小的、独立自主的组件比编写完整一大块代码要容易。
  简单的组件可以在设计、编码、单元测试之后置之不理——只需新增代码时，
  没必要一直改动已有的代码。
- 正交的方法同时促进了重用。
  如果组件有一个职责定义清晰的规范，就能以原作者预想不到的方式与新组件组合使用。
  系统耦合越松散，重新配置和改造系统就越容易。
- 组合正交组件能获得相当微妙的生产力提升。假设一个组件能做 M
  件独特的事情，另一个能做 N 件。如果它们是正交的，组合起来就能做 $M \times N$
  件事。
如果两个组件不正交，有重叠之处，结果就少一些。
在正交组件的组合过程中，单个单元的价值被放大了。
...
有时这些模块组件被组织到不同的层次上， 每一层都提供了一级抽象。
...
> 图
> 2-2：分层的系统架构。
> 自上而下依次为用户界面层，数据库访问、授权、业务逻辑层，框架与标准库层，
> 以及容器服务层。
> 每一层只使用其下一层提供的抽象。

可以用一个简单的方法来测试设计的正交性。 当你规划好组件后，
问问自己：如果某个特定功能背后的需求发生显著变化， 有多少模块会受影响？
...
事实上， 如果系统构造得足够精心， 应该可以用同一套底层代码支持这两种界面。
...
当你引入一个工具包（就算是来自同一团队其他成员的一个程序库）时，
问问自己， 它是否会将一些不应该有的变化强加给你的代码。
 如果对象持久化模式[^ch03-ch2-7]
...
还要监控软件的大环境。 如果不这样， 其他模块中的功能就很可能被无意间重复，
或是把已有的知识表达两次。
...
我们会在[话题 28：解耦](#28-解耦)中加以讨论。 如果你需要改变一个对象的状态，
...
除此之外就没有全局数据的概念）。 小心单例——其同样会导致不必要的耦合。
...
因为模块级别（或单元）的测试比集成测试更容易枚举和执行。 实际上，
我们建议将这些测试作为常规构建过程的一部分自动执行（参见
[话题 41：为编码测试](#41-为编码测试)）。

编写单元测试本身就是一个有趣的正交性测试。
要让单元测试构建出来并运行起来， 需要做些什么？ 需要导入系统其余的大部分代码吗？
...
提交并通过测试后， 要在回顾代码时把 Bug 修复标注出来。 你可以启用月度报告，
...
- 假设有一组具有图形用户界面的工具，以及一组小巧但可以组合使用的命令行工具，
  哪一组更具备正交性，为什么？
  对于刚好符合设计意图的场景，哪一组更容易使用？
  哪一组工具更容易和其他工具组合使用去应对新的挑战？
  哪一组更容易学习？
...
但如果你真的将数据库的概念抽象出去——让它只是以服务形式提供持久化
——现在你就可以灵活地中途换马了。
...
- 把云虚拟机换成容器再来一遍
- 基于云的无服务器架构
- 最后，无可避免地，有些任务又回到了大铁块。

继续， 把最新、最伟大的时髦玩意添加到这个列表中，
...
曳光弹和普通弹药间隔着压入弹夹。 当曳光弹发射时， 上面的磷就会被点燃，
...
就像枪手一样， 你在黑暗中尝试击中目标。 因为用户以前从未见过这样的系统，
所以需求可能是模糊的。 或许你正在使用不熟悉的算法、技术、语言或库，
...
它们能让我们快速、直观、可重复地从需求中得到最终系统的某个方面。
...
曳光弹也就变得更为重要。 对于我们来说， 最初的曳光弹就是创建一个简单的工程，
...
> 图
> 2-3：曳光弹穿越系统各层。
> 系统分为用户界面、授权、业务逻辑、数据模型、数据库五个架构层，
> 对角线表示某个特性依赖的代码路径，实心阴影部分需先实现，
> 弯曲线条部分（尚未实现）留待以后补充。

我们曾经接手过一个复杂的客户端-服务器数据库营销项目。
有一部分需求是能够指定并执行临时查询。 服务器是一系列关系数据库和专用数据库；
客户端 UI 是用语言 A 写的； 用于访问服务器的接口，
...
所能做的仅仅是提交一个列出表里所有行的查询。 但它提供了能与库交流的 UI，
...
而在系统有了明显的进展时则会欢呼鼓舞。 用户的持续认可， 和项目的推进一样，
...
项目赞助商和公司高层倾向于在最不合适的时候提出看看演示。 只要使用曳光代码，
你总是有东西可以演示。
...
这样就避免了在编写巨型代码块时， 一次次地在周报中汇报这部分已经完成了 95%。
...
可以展示给用户和开发人员。 随着时间的推移， 你会往这个框架中添加新的功能，
把留白的部分补充完整。 但是框架保持不变， 并且你知道系统将继续这样运作，
与一开始完成曳光代码时保持一致。
...
老派的人可能会用粘土模型来做风洞测试， 用软木和布基胶带为艺术部门做模型， 等等。
...
我们可以用不同的材料制造原型。 便签非常适合构建动态事物的原型，
...
错误检查可以不完整， 甚至完全没有也行。 如果你偏离了预定的航线，
...
如果使用得当， 原型有助于在开发的早期就识别出潜在的问题点，
并给予纠正——此时修正错误既廉价又容易。 这能帮你节省大量的时间和金钱，
极大地减少你的痛苦。
...
反之亦然。 与之相对， 问题域的语言也能反过来启发程序设计的解决方案，
而且我们认为这更为重要。

我们总是试图使用应用领域的词汇表来编写代码（参见
[话题 45：需求之坑](#45-需求之坑)）。
...
Cucumber[^ch03-ch2-13] 提供了一种与编程语言无关的方法来编写测试。
...
因此， RSpec
和路由代码是嵌入在运行代码之中的：它们对你的编码词汇表做了真正的扩展。
 Cucumber 和
Ansible 则被代码读取， 然后转换成代码可以使用的某种形式。
...
不必做大量元编程， 只需编写函数来完成这项工作即可。 事实上， RSpec
差不多就是这么干的：
...
- 你当前的项目中的一些需求是否可以用特定领域语言来表达？
  有没有可能写一个编译器或是一个翻译程序来生成所需的大部分代码？
- 如果决定采用小语言作为一种更接近问题域的编程方式，
  那么你就要做好准备多花一些力气来加以实现。
  将为一个项目开发的框架复用到别的项目，你能找到对应的方法吗？

#### 练习

练习 4（参考答案见附录）

我们希望用一种小语言来控制一个简单的绘图包（可能是一个海龟图形系统）。
...
用内部语言来实现。 不用其他小聪明， 为每个命令编写一个函数就够了。
...
通过学习估算， 把这项技能发展为对数量级的直觉，
你将能展现出一种魔法般的能力， 可以判别事情的可行性。
...
那么听的人往往觉得实际所需时间会很接近这个数字。 然而， 如果你说的是“哦， 大约 6
个月吧”， 他们就会认为还需要 5 到 7 个月不等。 两个数字表示的时间周期是一致的，
但是“130 天”却可能暗示了比你想象中更高的精度级别。
我们建议你采用下面的时间尺度做估算：
...
因此， 如做完功课后你确定项目将花掉 125 个工作日（25 周）， 那么你可以用“大约
6 个月”来描述这个估算。
...
在你全身心投入建模之前， 先问问周围有没有一些过去经历过类似情况的人。
看看他们的问题是如何解决的。 你不太可能找到一个完全匹配的事情，
但你会惊讶地发现， 总能成功地多次借鉴他人的经验。

#### 理解在问什么

任何估算工作的首要部分都是建立对所问内容的理解。
...
一旦参数被分离， 就可以将它们逐一过一遍， 为每个参数分配一个值。
这一步通常会引入一些误差。 但关键是找出哪些参数对结果影响最大，
集中精力确保其近乎正确。 一般来说， 就重要性而言， 把数值累加到结果上的参数，
...
[Shell 游戏](#17-shell-游戏)中会对此展开讨论。 有了材料和工作台，
...
就有了一种自描述的数据流， 而它和创建它的应用程序是相互独立的。

#### 什么是纯文本

纯文本由可打印字符组成， 以某种能传递信息的形态呈现。
它可以和购物清单一样简单：
...
人类可读、自描述的数据，
 比其他任何形式的数据以及创建它们的应用程序都更有生命力， 这毋庸置疑。
 只要数据还在，

就有机会用到——即使到那时， 产生数据的应用程序可能早已失效很久。
只需要有格式的部分知识， 就能解析纯文本文件； 但对于绝大多数二进制文件而言，
则必须了解整个格式的所有细节， 才能成功解析出来。

来看看这样一个数据文件， 它来自于某个遗留系统[^ch04-2]。
你对当初的应用程序知之甚少， 但你知道这个文件里存有客户的社会安全号码列表，
...
当系统崩溃时， 或许你只能在一个非常受限的环境中进行系统恢复（例如，
...
这样的文件比较工具能让你快速查看有哪些改动， sum 可以为文件生成一个校验和，
用于监控无意（或恶意）的修改。

#### 易于测试

如果你使用纯文本生成合成数据来驱动系统测试，
...
它们正在彼此间协商数据交换——即使这一天来临， 无处不在的文本文件仍然在发挥作用！
事实上， 在异构环境中使用纯文本， 利远大于弊。
当各方需要一个都能使用的公共标准才能相互沟通时，
纯文本就是这个标准。
...
对于操作文本文件的程序员来说， 工作台就是命令行 Shell。 在 Shell 中，
你可以调用所有可用的工具，
...
如果没有花很多时间研究所用系统的 Shell 命令， 那么上面这段可能会让你心存畏惧。
然而， 投入一些精力去熟悉 Shell， 事情很快就会步入正轨。 多试试 Shell 命令，
你会惊讶地发现工作效率提高了不少。
...
- 设置颜色主题。 为你的 Shell 把网上各种主题都试一遍会花掉很多时间。
...
- 命令补全。
   大多数 Shell 会补全命令和文件的名称——输入头几个字符， 按一下 tab 键，
  就会尽可能补全。 而且你还可以再进一步， 将 Shell
...
- 你目前在 GUI 中手动执行哪些操作？
  是否曾经给同事讲过这样的操作步骤——一连串“点这个按钮”“选这一项”式的独立操作？
  可以自动化吗？
...
但这还不是真正的好处——远不止于此。 如果你操作编辑器游刃有余，
最主要的收益来自于变得“顺畅”——不必再把心思花在编辑技巧上面。
从思考到把想法呈现在编辑器中， 整个过程没有阻塞， 一气呵成。
...
一旦你发掘出一个有用的新特性， 就要尽快把它内化成肌肉记忆，
这样在使用的时候就能不假思索。 据我们所知， 唯一的方法就是不断重复。
有意识地寻找机会使用这些新获取的超能力， 最好是一天多次。 一周左右，
你会发现自己已经在下意识地运用。

#### 培育你的编辑器

大多数强大的代码编辑器都是基于一个基础核心创建的，
之后可以用扩展的方式不断增强这一核心。 有些扩展是编辑器自带的，
有些则留待日后添加。

当你在使用编辑器过程中遇到明显的限制时， 可以四处找找有什么扩展可以解决问题。
极有可能你并非这个功能的唯一需求者， 如果幸运的话， 已经有人发布过解决方案。

更进一步， 深入研究一下编辑器的扩展语言。
弄明白怎样用它把一些重复的工作自动化——通常也就是一两行代码的事情。
...
- 寻求集成。 在写这一章时， Dave 担心他无法在编辑器里预览最终的布局（一个 PDF
  文件）。 下载了一个扩展之后， 排版结果就显示在原始文本旁边，
  两者都在编辑器中。 记录一个列表， 列出你想在编辑器中集成的东西，
  然后找到它们。

- 再有野心一些， 如果找不到想要的插件或扩展， 就自己写一个。 Andy
  喜欢为自己称手的编辑器编写 Wiki 插件。 如果你找不到这个插件， 自己去写一个！
...
在用户界面中， 我们最珍视的功能之一就是 Undo
键——一个可以原谅我们犯下错误的按钮。 如果环境支持多级撤销和重做就更好了，
这样可以从几分钟前发生的事情中恢复过来。

但如果错误发生在上周呢？ 在那之后， 你已经开关机数十次了。
这就是使用版本控制系统（VCS）的诸多好处之一：它是一个巨大的 Undo 键，
一个项目的时间机器， 可以让你回到上周的那些平静日子——那时，
代码明明还可以编译和运行。
...
然而这还不算完！ 有些人倒是在用版本控制，
但把仓库（保存着所有的修改历史）指向一个网络地址或是放在云存储中
——他们深信找到了两者兼顾的最佳方案：有个公共仓库可以随时随地往里面放东西，
...
对跟踪 Bug、审核、性能以及质量这些目标来说， 意义重大。

VCS 还能帮你标识出软件的不同发布版本。 一经标识，
就总是能回到特定的版本并重新生成它， 而不受之后可能发生的更改的影响。

版本控制系统会将所维护的所有文件保存在一个中央仓库中——这是一个理想的归档去处。
...
版本控制系统不只是保存着一份项目的单一历史。 它最强大和最有用的特性之一，
就是能让你把开发过程中的一座座孤岛隔离到所谓的分支中。
你可以在项目历史的任何时间点创建分支（这种行为有时被称为分叉），
然后你在这个分支上所做的任何工作就都已和其他分支隔离开。 在未来的某个时间，
你可以将手头上的分支合并回另一个分支，
这样目标分支就包含了你在分叉出来的分支上所做的修改。
甚至多个人可以在同一个分支上工作：在某种程度上， 分支就像项目的一个小型克隆。

分支的第一个好处是为你提供了隔离。 如果你在一个分支中开发了特性 A，
而另一个团队成员在另一个分支上开发了特性 B， 你们之间不会相互干扰。

第二个好处可能令人惊讶：团队项目的工作流通常围绕分支来组织。

如何在工作流中用好分支， 可能会让人有些困惑。
用分支进行版本控制和组织测试有一些共同之处：关于这二者，
有无数人排队等着告诉你该怎么做。 但这样的建议很大程度上毫无意义，
因为他们等于在说“这么做在我这里很有效。”
...
好在， 机器上所有影响配置和使用的数据， 原本就保存在版本控制系统中， 包括：
...
虽然版本控制对个人项目极其有用， 但它在团队工作中才能真正发挥作用。
而其价值很大程度上来自于如何托管仓库。
...
但这很可能不是你的核心业务， 对大多数人而言， 建议使用第三方托管。
要找支持以下功能的：
...
很多团队都会对 VCS 进行配置， 使其有一个特定的分支， 只要有变更推送进去，
...
- 知道可以使用 VCS 回滚到任何以前的状态是一回事， 但是你真的可以做到吗？
  知道正确的操作命令吗？ 现在就开始学习吧， 而不要等到灾难来临时背负着压力去学。
...
Bug——名副其实的 Bug， 一只飞进早期计算机系统中的蛾子。
当被要求解释那台机器为什么不能正常工作时， 技术人员报告“在系统中发现了一只
Bug”， 并且很尽职地把它钉在了记录本中， 包括翅膀等所有部分。

遗憾的是， 我们的系统总是会有 Bug， 虽然不是会飞的那种。 在 14 世纪， Bug
这个词用来指称魔鬼， 或许这个古义放到现在更加贴切。 软件缺陷以各种方式表现出来，
从对需求的误解到编码的错误。 不幸的是， 现代计算机系统仍有局限性，
能做你让它做的事， 却不一定做你想让它做的事。
...
而不是拿出攻克待解难题的态度来对待。
要接受这样一个事实：调试不过是解决问题而已。

有人在发现了别人的 Bug 之后， 会不遗余力地把责任推到罪魁祸首身上。
...
或是在老板或客户紧张地站在身后的注视下， 拼命寻找问题原因的时候。 然而，
...
调试时要注意别短视。 不要仅仅去纠正眼前的症状：
实际的错误很可能与你观察到的现象还有几步之遥， 并且可能牵涉许多其他相关的事情。
...
- 你可能需要访谈报告 Bug 的用户， 这样才能收集到比最初提供的更多的数据。
- 人为的测试（例如程序员从下到上画线）对应用程序的测试而言还不够。
  你必须粗暴地测试所有边界条件， 并且重现真实最终用户的使用模式。
...
但是， 我们可不想通过一系列步骤来重现一个 Bug， 我们想让它单条命令就能重现。
...
有时， 只要强制自己对暴露 Bug 的环境进行隔离， 就能洞察该如何修复它。
写测试这个动作， 就会揭示出解决方案。
...
并在生产环境的第一周里运行良好。 之后， 当输入特定数据集时， 它会突然崩溃。

你可以着眼于它崩溃的地方， 试着由此回溯。 不过有时从数据着手更容易一些。
获取数据集的副本， 并用这份数据测试本地运行的应用程序副本，
 确保其在本地也能崩溃。
然后用二分法对数据逐次对半划分， 直到能准确分离出导致崩溃的输入值。

#### 版本间回退

一个很棒的团队， 将软件好好地发布到生产环境中， 某一天代码却出了 Bug，
而软件在一周前还能正常工作。 有什么方法能帮你找出是哪一步的变更引入了
Bug？ 你猜怎么着， 二分法要登场了。
...
这样做有时找到的是一个更大一点的值， 这可能意味着你要找的值不在数组里面。
...
跟踪语句是一些小的诊断消息， 会打印到屏幕或文件中， 其中包含诸如“已到这里”和
“x 的值=2”之类的内容。 与 IDE 风格的调试器相比， 这是一种原始的技术，
但是它在处理诊断调试器无能为力的那几类错误时， 特别有效。 对于任何系统，
当时间本身就是一个影响因素时， 比如并发进程、实时系统和基于事件的应用程序，
跟踪信息都是不可或缺的。
...
那就是向其他人解释该问题。
你找的人只需要越过你的肩膀看屏幕并不停点头（就像一只在浴缸里上下跳动的橡皮鸭），
...
假定应用程序代码对库的调用不正确， 要比假定库本身坏掉了更有效。
...
有时改变的东西超出了你的控制范围：更新操作系统、编译器、
数据库或其他第三方软件的版本，
可能会破坏以前正确的代码， 因而出现新的 Bug。 如果之前发现过 Bug，
...
当你发现自己被一个 Bug 吓了一跳时（甚至是压着嗓子嘀咕一句“这不可能”），
...
就对与它有关的 Bug 置之不理。 你需要证明它没问题， 用出 Bug
...
是否需要修改单元测试或其他测试， 以便让这些测试能够捕获到它。

同样， 如果 Bug 是损坏的数据造成的结果，
 而数据在引爆程序之前经过了好几层的传播，
可以试试给函数加上更完备的参数检查，
以便更早地将问题剥离出来（关于尽早崩溃和断言的讨论，
分别参见[话题 24：死掉的程序不会说谎](#24-死掉的程序不会说谎)和
[话题 25：断言式编程](#25-断言式编程)）。

如果一个地方出现了 Bug， 代码的其他地方是否也可能受到影响？
现在是找到并修复那些地方的时候了。 确保不管发生了什么，
你都能知道这事是否还会发生。
...
还有很多。 Pragmatic Bookshelf 就是用这种实实在在的方式，
...
定为了标准， 因此现在有一堆 .yaml 文件需要转换成 .json。 需要编写一个脚本，
然后接收一个目录，
 把其中每个 .yaml 文件转换为相应的 .json 文件（因此 database.yaml
...
代码可能不符合我们的高标准， 也可能需要处理未必有效的输入。
所以我们被教导要进行防御式编程——有任何疑问， 都要去验证我们得到的一切信息；
...
[断言式编程](#25-断言式编程)中描述了一种沿路检查的简单方法
——为你所做的假设编写主动校验的代码。

当你的程序变得更加动态时， 你就会发现自己像在玩抛球杂耍一样，
...
例程和任何潜在调用者之间的契约因此可以解读为：

> 如果调用者满足了例程的所有前置条件，
> 则例程应保证在完成时所有后置条件和不变式都为真。

如果任何一方未能履行契约， 就会执行事先约定的补救措施——可能是抛出异常，
或者程序终止。 不管发生什么事， 毫无疑问， 不能履行契约都是一个 Bug。
这是不应该发生的事情， 也正因如此，
不应该使用前置条件来执行诸如用户输入验证之类的操作。

既支持前置条件和后置条件， 也可通过 spec 提供更全面的监控设施。
下面这个示例是一个用于存款的银行函数， 使用了简单的前置和后置条件：
...
如果你在调用 accept-deposit 函数时， 为一个有效账户传入了正数金额，
那么它将创建相应类型的交易， 并执行相关处理操作。 然而， 如果程序中有一个
...
在这个示例中， 用足够大的金额调用 accept_deposit 函数可能会触发额外的处理步骤。
...
这是一种比简单地检查输入更好的方法； 在这个示例中， 如果参数超出范围，
就无法调用此函数了。
...
这个概念指的就是状态。 在面向对象语言中， 状态与类的实例相关联。
但是其他语言中也有状态。
...
对于不支持在代码中实现 DBC 的语言， 你或许只能止步于此——这也不算太糟。 DBC
毕竟是一种设计技术。 即使没有自动检查，
...
此外， 也没有内建的变量“旧”值的概念； 旧值指的是变量在方法入口处的值。
如果使用断言来实施契约， 即使语言允许这样做， 也必须在前置条件中添加代码，
以保存希望在后置条件中使用的信息。 在 DBC 的诞生地 Eiffel 语言中， 你只需要使用
old 表达式就够了。
...
sqrt 将返回特殊值 NaN（Not a Number， 非数字）。
 而你可能要到程序稍后， 尝试对 NaN
进行数学运算时， 才会看到令人惊讶的结果。
...
当检查被内建为语言的一部分时， 答案是两者都不是： 前置条件在幕后被测试——
发生在调用者调用例程之后、进入例程之前。 因此，
...
我们曾经写过一个借记卡交易交换系统。 一个主要要求是，
借记卡用户账户中的同一笔交易不应该发生两次。 换句话说，
不管可能发生哪种类型的失败， 出错时的处理方式都应该是不处理交易，
而不是重复处理交易。
...
但现在我们只需要一个控制它的界面。 它有十档速度设置（0 表示关闭）。
...
可能有一个网络错误或文件系统错误没有被捕获， 然后得到了空的或损坏的数据。
 也可能是几百万条指令之前的逻辑错误导致了待选的 case 语句不再是预期的 1、2 或 3
中的一个； 我们意外地进入了 default 分支。 这也是为什么每个 case/switch
语句段都需要有一个 default 子句的原因之一： 我们想知道什么时候“不可能”发生了。

人们很容易陷入“这不可能”的心态。 我们大多数人编写的代码，
都没有检查文件关闭操作是否成功， 或者确认 trace 语句是否按预期写入输出。
这些事情都有一个共性， 似乎我们不必这样做——在所有正常情况下，
...
是一种很好的做法。 他们的代码中到处是这样的东西（一个“裸”的 raise
语句重新抛出当前的异常）：
...
否则， 就可能继续把损坏的数据写入重要的数据库， 或是命令洗衣机进入 20
个连续旋转周期。
...
该技术非常有效， 也有助于解释这些语言为何常被用于高可用、容错的系统中。

在其他环境中， 简单地退出正在运行的程序， 可能并不合适。
你可能已经占用了一些资源， 它们可能无法被释放，
...
运用到需求、设计、代码、注释及所做的任何事情中。 它念起来是这样的：
...
任何时候你发现自己在想“这当然不可能发生”， 就添加代码来检查它。
...
对于任何复杂的程序， 所有执行路径中你能测试到的，
充其量只是很小的一部分。 其次， 乐观主义者忘记了程序是在一个危险的世界中运行的。
...
无论在什么时候， 我们写代码都要管理资源：
内存、事务、线程、网络连接、文件、计时器——所有可用数量有限的东西。
大多数情况下， 资源使用遵循一个可预测的模式： 分配资源， 使用它， 然后释放它。

然而， 许多开发人员对资源分配和释放的处理并没有一致的方案。 因此，
这里有一条简单的建议：

> [!TIP]
> **提示 40 有始有终**

这条建议可以应用到大多数场合。 简单来说， 分配了资源的函数或对象，
也应负责释放它。 让我们通过一些糟糕的代码来演示如何应用它——这是一段
...
整洁之下掩盖着一个主要问题。 例程 read_customer 和 write_customer
...
而这个共享变量甚至没有出现在 update_customer 例程中。
...
报的错误是打开了太多的文件。 这是因为在某些场合下， write_customer 未被调用。
当这种情况发生时， 文件没能关闭。

有一个非常糟糕的解决方案可以应付这个问题， 那就是针对 update_customer
中的特定情况做一些处理：
...
可以将资源的生命周期限定在某种封闭的代码块内。 在 Ruby 中， 文件的 open
方法有一种变体用法， 它把打开的文件引用传入一个代码块， 也就是 do 和 end
之间的代码块：
...
在本例中， 代码块的末尾， file 变量超出作用域时会关闭外部文件。 这样，
就可以确保文件一定会被关闭， 而无须记得手动去释放资源。
...
对于一次需要多个资源的例程， 可以对资源分配的基本模式做一些扩展。
只有两个额外的建议：

- 释放资源的顺序与分配资源的顺序相反。 在这样的次序下，
  如果一个资源包含对另一个资源的引用， 就不会导致被依赖的资源被提前释放。
- 在代码的不同位置， 如果都会分配同一组资源， 就始终以相同的顺序分配它们。
  这将减少死锁的可能性。 （如果在 A 进程中获取到 resource1，
  并在持有它的前提下再去请求 resource2， 与此同时， B 进程获取到 resource2，
...
是否有类似的机制来清除过期的日志？ 对于你创建的任何占用有限资源的东西，
请考虑一下如何保持它的平衡。
...
类代表一种资源， 构造函数会把该资源类型的一个对象交给你，
析构函数则在它超出作用域时将其移除。

如果使用面向对象的语言进行编程， 你可能会发现将资源封装到类中非常有用。
每次需要特定的资源类型时， 就实例化该类的对象。
当对象超出作用域或被垃圾回收器回收时， 对象的析构函数将释放封装起来的资源。

在你使用的语言中， 如果异常机制可能干扰资源的释放， 这种方法的好处尤其明显。

#### 保持平衡与异常

支持异常的语言会使资源释放变得棘手。 如果抛出异常，
如何保证在异常抛出之前分配的所有内容都已被依次释放？
 答案在一定程度上取决于语言支持。
你通常有两个选择：
...
写代码去实际检查资源有没有被恰当地释放， 绝对是一个好主意。
对于大多数应用程序， 我们可以为每种类型的资源都做一个封装器，
使用这些封装器去跟踪所有的分配和释放操作。 在代码中的某些特定位置，
程序逻辑能指明资源应处于某种状态： 用封装器去检查是否确实如此。 例如，
长期运行的处理请求的服务程序， 都可能在其主循环的顶部有一个单点，
程序会在此等待下一个请求的到来。 这就是一个好位置，
可以在这里确认资源的用量没有较上一个执行循环持续递增。
...
这名警官指的是司机在前灯照射范围内及时停车或转向的能力。

前灯有一定的照射范围， 被称为投射距离。 过了临界点， 光线就会变得太分散，
...
但是， 似乎已经听到你在抗议——我们不是应该为将来的维护做设计吗？ 没错，
不过要适可而止： 别超过你能看见的范围。 越是必须预测未来会怎样，
就越有可能犯错。 与其浪费精力为不确定的未来做设计， 还不如将代码设计成可替换的。
当你想要丢弃代码， 或将其换成更合适的实现时， 要让这一切变得无比容易。
使代码可替换， 还有助于提高内聚性、解耦和 DRY， 从而实现更好的总体设计。

即使你对未来充满信心， 也总有机会遇到黑天鹅。

#### 黑天鹅

纳西姆·尼古拉斯·塔勒布在他的《黑天鹅：如何应对不可知的未来》[Tal10] 一书中提出，
历史上所有重大事件都来自影响巨大、难以预测、罕见且超出正常预期的事件。
这些异常值虽然在统计上很少见， 但却具有远超占比的影响。 此外，
认知偏见往往使我们对悄然逼近工作的变化视而不见（参见
[石头做的汤和煮熟的青蛙](#4-石头做的汤和煮熟的青蛙)）。

大约在《程序员修炼之道》的第一版出版前后，
计算机杂志和在线论坛上有一个热门话题引发了激烈的辩论： “谁将赢得桌面
...
传统的过程式代码和面向对象代码可能耦合得过于紧密，与你的目标不一致。
...
该对象具备读取器和设置器，可以用来计算总金额和折扣—— 隐含的知识实在太多。
但更糟糕的是，将来有很多事情是不能改变的，否则代码无法继续工作下去。
火车的所有车厢都是耦合在一起的，
所有的方法和属性就像那些列车车厢一样，紧紧连在一起。
...
在随便哪个地方的代码块中，都可能设置 totals 对象的字段，
只要有维护者不守规矩，代码就无法按照新的折扣策略全部修正。
...
订单对象和订单的总金额之间也有相同的问题。 订单的实现使用了一个单独的对象 totals
...
子类化，是指一个类从另一个类继承状态和行为， 对它的误用是个重要话题。
...
但是比起之前的代码，现在的代码做了更多的事。
每次转换的结果都是一个新的状态和一个动作的名称。
在回到循环之前，会根据动作名称选择要运行的代码。

这段代码非常基础，但足以完成任务。 还有许多其他变体：
转换表可以是动作对应的匿名函数或函数指针，
你可以将实现状态机的代码包装在一个独立的、自带状态的类中，等等。

这里并没有“必须在同一时间处理所有状态转换”的限制。
如果你曾实现过在应用程序中注册用户的流程，就知道可能会有好几个步骤，
...
状态机并没有被开发人员充分利用，我们鼓励你找机会多加使用。
但其并不能解决所有与事件相关的问题。 对于杂耍般抛接事件球时会遇到的其他问题，
我们接下来将讨论其他解决方法。

#### 观察者模式

在观察者模式中，我们有一个事件源，称为被观察对象；
而对其中的事件感兴趣的客户列表，则被称为观察者。
...
我们可以对它们进行操作、合并、过滤，以及做我们熟知的所有其他数据处理。
我们甚至可以将事件流和常规集合组合在一起。
流可以是异步的，这意味着你的代码有机会按事件到来的方式去回应事件。

响应式事件处理方法的事实标准已经在相应网站上做出定义
（网址参见链接列表 5.2 条目），
其中包含一组与语言无关的原则，并记录了一些常见的实现。
 在这里，我们选用 JavaScript
的 RxJs 库。
...
在前面的例子中，我们的用户 ID 列表（在被观察对象 users 中）是静态的。
但不一定非得如此。 也许我们想在人们登录网站时收集这些信息。
我们所要做的就是在创建会话时生成一个发出用户 ID 的被观察对象，
并用这个被观察对象来取代那个静态对象。 这样，我们就可以在收到这些 ID
的同时，获取用户的详细信息， 大概还能将信息存储到某个位置去。

这是一个非常强大的抽象方法：我们不再需要将时间视为必须管理的东西。
事件流将同步和异步处理统一到一个通用的、方便的 API 之后。

#### 事件无处不在

事件到处都是。 有些是显而易见的：一次按钮点击，一个计时器到期。
有些则不那么明显：有人登录进来，文件中的一行匹配了一个模式。
...
为了开始我们的研究，先坐上时间机器回到 20 世纪 70 年代， 让一位 UNIX
程序员为我们编写一个程序，
列出目录树中最长的五个文件，这里的最长表示“拥有最多的行数”。

你可能以为他们会打开编辑器并开始输入 C 代码。
但事实并非如此，因为他们考虑的是我们所拥有的（目录树）
...
现在你已经把整个程序表示为一个函数。 接下来就可以找出从输入到输出的各个步骤。
这是一个自顶向下的方法。
...
它所做的就是将左侧的值作为右侧函数的参数，插入到第一个参数的位置。 因此，
...
在变换式模型中，我们将其颠倒过来。
不要把数据看作遍布整个系统的小数据池，而要把数据看作一条浩浩荡荡的河流。
...
该结构可以告知其中所包含的值是否有效。 例如，在 Haskell 中，这个包装器被称为
Maybe。 在 F# 和 Scala 中是 Option。

如何使用这个概念取决于语言。 但是，通常有两种编写代码的基本方法：
你可以在变换的内部或外部做错误检查。
...
我们需要为包装器选定一种表示形式（包含错误提示或错误码的数据结构）。
...
通过这种方式，函数能简单地沿着管道传递错误。 truncate_lines 做的也是同样的事情。
...
但事实是，它不允许我们添加类似的东西， 这种做法或许只能用于许多其他语言。

我们面临的问题是，当错误发生时， 我们不希望继续运行管道后面的代码，
也不希望让后面的代码感知到错误正在发生。 这意味着我们需要推迟管道函数的运行，
直到我们知道管道中先前的步骤已经完成。
...
and_then 函数是绑定函数的一个示例： 绑定函数会把包装在容器中的值取出来，
并对其应用一个函数，再返回一个新的包装过的值。 管道函数在使用 and_then
时要多带一些标点符号， 这是因为 Elixir
用这些符号来表明，需要将函数调用转换为函数值。 变换函数因此变得简单：
...
方法认为继承是一种将类型组合起来的方式， 这种方式在 C++ 和 Java
...
因为在名字消歧方面的一些值得商榷的决定， C++ 在 20 世纪 90
...
大多数 OO 语言都允许你指定一个类实现一个或多个行为集。 例如，Car 类实现了
...
和 stop 这两个方法，
 而实现 Locatable 的类必须实现 getLocation 和 locationIsValid。
这意味着，我们之前的 Car 类定义，只有在包含所有这四个方法时才有效。

接口与协议之所以如此强大， 是因为我们可以将它们用作类型，
而实现了相应接口的任何类都将与该类型兼容。
...
[^ch06-1]: 所以准确地说它不是法则，更像是得墨忒耳式的好点子之类的东西。
...
[^ch06-5]: 管道的最早使用似乎要追溯到 1994 年，在关于语言 Isabelle/ML
...
时间是软件架构中经常被忽略的一个方面。
唯一让我们担心时间的时候，是日程表上的期限、距发布还剩的天数——
不过这不是我们这里想讨论的内容。
我们讨论的是时间作为软件本身的设计元素所起的作用。
时间对我们来说有两个重要的方面：
...
但这样的思考方式会导向时域耦合——时间上的耦合： A 方法必须在 B
方法之前调用； 一次只能运行一个报告； 在按钮按下前必须等屏幕先重绘；
“嘀嘀”一定在“嗒嗒”之前发生。

这个方法不太灵活，也不太符合现实。

我们需要考虑并发性，并解除基于时间或顺序的耦合。
这样做之后，我们可以在开发的许多领域获得灵活性，并减少基于时间的依赖：
工作流分析、体系结构、设计和部署。
其结果是系统更容易推理，并且可能更快、更可靠。

#### 搜寻并发性

在许多项目中，我们需要对应用程序的工作流建模并加以分析，这是设计工作的一部分。
我们想知道哪些事情可以同时进行，哪些必须按照严格的顺序发生。
有一种方法可以做到：使用活动图这类记号来理清工作流。[^ch07-2]
...
然而，如果酒保真的按这份清单一步步操作，肯定会丢掉工作。
尽管操作是依次描述下来的，但其中许多步骤可以并行执行。
可以用下面的活动图来找出并分析其中的并发性。
...
看清依赖究竟在哪里，会让人眼前一亮。 在这个实例中，顶层任务（1、2、4、10 和
...
把陈列柜换成联名银行账户，把服务员换成 POS 机。
你和你的伴侣同时决定买一部新手机，但账户里的钱只够买一部。
那么总有一方——银行、商店，或者你——会非常不高兴。
...
这两个服务员在并发地工作（在现实生活中则是并行）。 我们来看看他们的代码：
...
服务员 1 清点了一下当前派的数量，发现是 1，于是向顾客承诺有派。
但与此同时，服务员 2 也跑了过来。 她看到的数量同样是
1，并向她的顾客做出了相同的承诺。 然后，其中一人拿起了最后一块派，
另一个服务员会进入某种错误状态（这可能招致很多赔礼道歉）。

这里的问题，不是两个进程可以同时向同一块内存写入数据。
而是没有哪个进程能保证自己看到的内存内容是一致的。 实际上，当一个服务员执行
...
让我们做出改变，把访问改由集中控制。 要做到这一点，我们必须改变 API，
...
这段代码暴露了一个常见的误解。 我们已经把资源访问转移到一个中心位置，
...
这两个选项我们都不想选。 务实的思路是把“派加冰淇淋”本身作为一种资源。
将这段代码移到一个新模块中，
...
以便能利用线程并行地完成更多的工作。
这导致构建失败，错误以奇怪的方式随机出现。
这些错误有一条共同的线索： 找不到文件或目录，尽管它们确实位于正确的位置。

我们跟踪到几处会临时修改当前目录的代码。
在非并行版本中，只要代码运行完后恢复之前的目录就足够了。
但在并行版本中，一个线程会修改当前目录，
而与此同时，另一个线程恰好将在这个目录下启动。
新启动的线程本应在原始目录下运行，
...
它们可能叫作互斥量、监视器或信号量，通常以库的形式实现。

然而，有些语言本身就内置了并发支持。 例如，Rust 强化了数据所有权的概念；
一次只能有一个变量或参数包含对任何特定可变数据段的引用。

你也可以说，函数式语言倾向于让所有数据不可变，从而简化了并发。
然而，函数式语言仍然面临着同样的挑战，
因为在某个时刻终将踏入那个可变的现实世界。
...
接下来的部分会提供一些替代方法，让你无痛苦地利用并发带来的优势。
...
角色与进程为实现并发性提供了一种有趣的方式， 而且无须同步访问共享内存。[^ch07-4]
...
让我们把角色概念套用到上面的餐厅例子中。
这个例子中将有三个角色（顾客、服务员和装派的柜子）。

整个消息流如下所示：

- 我们（充当某种外部的神明）让顾客感到饥饿
- 作为回应，他们会向服务员点派
- 服务员会向柜子要一些派给顾客
- 如果有派，柜子会把派送到顾客那里，并通知服务员把它加到账单中
- 如果派已经没有了，柜子会告知服务员，让服务员去给顾客道歉

我们选择用 JavaScript 的 Nact[^ch07-5] 来实现代码。 我们为此添加了一个小包装器，
以便能以简单对象的形式编写角色， 其中键是它接收到的消息类型，
值是在接收到特定消息时要运行的函数。
...
但在这个示例中，我们将力求简单，手动启动角色。 同时也会直接传入一些初始状态：
...
- 你当前是否还在写用互斥量来保护共享数据的代码？
  为什么不试试用角色模型为同样的代码做一个原型呢？
- 餐厅的角色代码只支持订购派。 把它扩展一下，让顾客能订购派加冰淇淋，
  由不同的角色分别管理派的分发和冰淇淋勺。
...
这是一种自由奔放的并发形式。 侦探是独立的进程、代理人、角色等。
...
> 图
> 6-3：黑板示例。
> 有人发现了矮胖子的赌债和电话记录之间的联系，也许他接到了恐吓电话。
> 黑板上贴着照片、蛋壳碎片、涂鸦、通话记录、目击证人、妻子不在场证明等线索，
> 福尔摩斯、马普尔、斯佩德三位侦探各自独立地查看和添加信息。

大卫·盖勒特的 Linda 是最早的黑板系统之一。 它将事实存储为带类型的元组。
应用程序可以将新的元组写入 Linda， 也可以用模式匹配取出已有的元组。

后来出现了分布式的类黑板系统，比如 JavaSpaces 及 T Spaces。
在这些系统里，你可以在黑板上保存活动的 Java 对象，而不仅仅是数据。
可通过部分匹配字段（借助模板和通配符）或是根据子类型，检索出对象。
例如，假设你有一个 Author 类型，它是 Person 的子类型。 你可以用一个 LastName
值为 “Shakespeare” 的 Author 模板，在包含 Person 对象的黑板中搜寻。 你会找到名为
...
管理这一领域的法律极其复杂，联邦、州和地方政府都有各自的要求。
贷款人必须证明自己已披露了某些信息， 还必须问过某些特定问题——
...
在编写本书第二版时， 许多应用程序都是用小型的、解耦的服务构建起来的，
...
角色、黑板和微服务这类架构级的解决方案， 帮助应用程序消除了潜在的并发问题。
但这种好处是有代价的。 这些方法比较难推理出来，因为很多操作都是间接的。
你会发现，为消息格式和（或）API 保留一个中央仓库应该很有帮助，
如果它还能生成代码和文档就更好了。 你还需要开发出好工具，
以便在消息和事实穿行于系统时跟踪它们。
（有一种有用的技术是，在启动特定的业务功能时， 为该功能加上唯一的跟踪 id，然后将
id 传播给所有相关的参与者。 最后，就能从日志文件中重建所发生的事情。）

最后，这类系统的部署和管理比较麻烦，因为活动部件更多。
但这未必是坏事： 在一定程度上，系统粒度也变得更细，
...
编码不是机械工作。 否则， 早在 20 世纪 80 年代，
人们曾寄予厚望的那些 CASE（电脑辅助软件工程）工具， 就已经取代程序员的工作了。
...
但听从直觉并不意味着就能全凭自动驾驶。 没有对代码进行积极思考的开发者，
...
也许你只是创造了一个只会招来 Bug 的蚂蚁农场。 不管是什么原因，
...
打算要做的事情， 并没有那么重要。 做原型就是一种可行的办法。

#### 游戏时间

Andy 和 Dave 都曾连着几个小时盯着空空如也的编辑器。 我们先输入一些代码，
...
如果你正在和现有的代码较劲， 那么就先把它放到一边， 然后做一些类似的原型。
...
有时候， 如果一个设计让你感觉不妥， 或是一些需求让你觉得不对劲，
...
记住那个士兵的故事， 下结论时要保持警惕， 以免出错。 我们应该避免靠巧合编程，
...
Fred 不知道为什么代码会出错， 因为他一开始就不知道它为什么能工作。
之前所做的有限“测试”似乎能表明工作正常， 但那只是一个巧合，
一个让他看上去备受鼓舞实则头脑发昏的假象。 现在， 只要你足够机警，
也能觉察到几个像 Fred 这样的人， 但你更需要对自己警醒——我们从不依赖巧合，
 真的吗？
...
下面 Fred 写的这段代码， 看起来像是想通过折腾一个特别的 GUI 渲染框架，
让屏幕上显示点什么：
...
但这些例程从来就不是这样设计的——虽然这么折腾似乎能让它们跑起来，
但这真的只是一个巧合。

更糟糕的是， 当场景最终被画出来时， Fred 不会回头去清理那些莫名其妙的调用。
“现在能工作了， 最好别碰这块……”
...
对于你调用的例程， 只应依赖文档记载的行为。 如果你做不到， 不管出于什么原因，
那就把你的假设记录下来。

#### 和结果接近是不够的

我们曾经在一个大型项目中工作过，
该项目上报的数据来自现场数量庞大的硬件数据收集装置。
这些装置分布在各个州和时区， 由于各种后勤和历史原因，
每个装置都被设定为当地时间。[^ch08-2]
由于时区解释的冲突和夏令时政策的不一致问题， 结果几乎总是错误的， 但错得不多，
只是数值上差 1。 项目的开发人员已经养成习惯，
通过简单地加上或减去 1 来得到正确答案， 他们认为反正在这种情况下只是差 1 而已。
而到了下一个函数， 又发现数值朝另一个方向产生偏差， 只好再纠正回来。

但事实是， 某些时候“只是”偏差了 1， 其实是一个巧合，
它掩盖了一个更深层的、更根本的缺陷。 如果没有恰当的时间处理模型，
整个庞大的代码库就会随着时间的推移而退化， 直到难以维持大量的 +1 和 -1 语句。
 最终，
没有一处是正确的， 这个项目被废弃了。
...
俄罗斯领导人总是在秃顶者和头发浓密者之间交替： 近两百年来，
俄罗斯会由一位秃顶（或正在秃）的国家领导人去接替一位不秃顶
（“头发浓密”）的国家领导人，
反之亦然。[^ch08-3]
...
这可能是一个难以诊断的竞态条件， 也可能是一个常见的普通 Bug。

至于在你的机器上通过而在服务器上没有通过的测试，
至于在你的机器上通过而在服务器上未通过的测试，
...
仅仅因为当前是在为 GUI 环境编写代码， 这个模块就必须依赖当前的 GUI 环境吗？
...
充满了虚假的因果关系和巧合的结果， 很容易假定 X 是导致 Y 的原因。
但是正如我们在[话题 20：调试](#20-调试)中所说的： 不要假设， 要证明。

在所有层次上， 人们都在头脑中使用许多假设——但这些假设很少被记录下来，
还经常在不同开发人员之间相互冲突。 不以既定事实为基础的假设，
是所有项目的祸根。
...
- 时刻注意你在做什么。 Fred 是慢慢让事情失去控制的， 直到最后，
  像[话题 4：石头做的汤和煮熟的青蛙](#4-石头做的汤和煮熟的青蛙)
中的青蛙一样被煮熟。
...
- 不要成为历史的奴隶。 不要让现有的代码去支配未来的代码。 如果不再合适，
  ...
  也不要让已经做完的事情限制下一步要做的事情——准备好重构（参见
[话题 40：重构](#40-重构)）。
  这个决定可能会影响项目进度。
 这里的假设是， 其影响小于不做更改所付出的代价。
[^ch08-5]

所以下次碰到有什么事情看起来可行， 但你不知道为什么， 先确认它不是巧合。
...
当键是 DepositAccount 时， 对应的值是一个账号字符串：
...
ITU（国际电信联盟）规定电话号码不应超过 15 位数字，
因此你可以将联系人的电话号码存储在保证至少包含 15 位数字的数字字段中。
你已经在整个北美进行了全面的测试， 一切似乎都很好，
但突然收到了来自世界其他地方的一连串投诉。 为什么？

练习 27（参考答案见[练习的参考答案](#练习的参考答案)）

你编写过一款应用程序， 可以为一艘可容纳 5,000 人的游轮上的餐厅提供常用食谱。
但有人抱怨说， 单位转换不精确。 你检查了一下，
代码使用了 1 加仑等于 16 杯这个转换公式。 这没问题， 不是吗？
...
你知道程序跑 1,000 条记录需要多长时间， 但如果扩展到 1,000,000 条记录会怎样呢？
哪些部分的代码需要优化？
...
某个算法处理 10 个元素只需一分钟， 但处理 100 个元素却可能要花一辈子。

我们发现， 每当编写任何包含循环或递归调用的东西时，
我们都会下意识地检查运行时间和内存需求。 很多时候这并不是一个正式的过程，
我们往往只是想快速确认一下， 所做的事情在这种情况下是否合理。 然而，
我们有时会发现自己正在进行更详细的分析。 这时大 O 符号就派上用场了。

#### 大O符号

大 O 符号， 写作 $O()$， 它是一种处理近似的数学方法。 当我们把一个对 $n$
条记录排序的特定例程写作 $O(n^{2})$ 时间时， 其实是在说： 在最糟糕的情况下，
时间会随 $n$ 的平方变化。 记录数量翻倍， 所需时间粗略估计会变为四倍。 可以把
$O$ 的含义看作是数量级。
...
1,000 倍， 但你从符号中看不出来。 大 O
永远不会告诉你时间、内存或其他资源的真正开销数值：
它只是告诉你数值会怎样跟着输入的量而变化。

图 7-1 直观展示了几种常见的 $O()$ 符号，
分类比较了它们各自的运行时间。 很明显， 从 $O(n^2)$ 开始，
事情很快就失控了。

例如， 假设有一个例程需要 1 s 处理 100 条记录。 那么处理 1,000 条需要多久？
如果你写的代码是 $O(1)$， 那么答案就还是 1 s。 如果是 $O(\lg n)$，
那么你可能要等大约 3 s。 对于 $O(n)$， 就会线性扩展到 10 s， 而 $O(n \lg n)$
则会花掉 33 s。
 如果你很不幸地写出一个 $O(n^2)$ 的例程， 那就得坐下来耐心等上 100 s
而如果使用的是一个指数算法 $O(2^n)$， 例程大概需要 $10^{263}$
年跑完——而你可能只是想去煮一杯咖啡。
...
> $n$，纵轴为运行时间。
> $O(1)$（数组访问）几乎平坦；$O(\lg n)$（二分查找）增长缓慢；
> $O(n)$（顺序查找）线性增长；$O(n \lg n)$（堆排序查找）略陡；
> $O(n^2)$（嵌套数组）急剧上升；$O(C^n)$（集合覆盖）几乎垂直。
...
只要你发现自己在写一个简单的循环， 就该明白这是一个 $O(n)$ 的算法。
如果循环中又套了一层循环， 它就是 $O(m \times n)$ 的。
这时应该问问自己这些值能有多大。 如果这些数字是有界的，
那么就能知道代码运行大概需要多长时间。
如果这些数字依赖于外部因素（例如通宵批处理要处理的记录数量，
 或是名单中的人名数量），
...
沉浸在理论中时， 不要忘记考虑实际情况。 在输入集较小的时候，
运行时间可能呈线性上升。 可一旦灌入百万级的输入， 系统就会颠簸不堪，
最终时间变得很长。 如果用随机输入去测试一个排序例程，
那么在第一次遇到有序输入时， 你可能会感到惊讶。 理论和实践要尽量都覆盖到。
...
- 每个开发者都应该了解如何设计和分析算法。
  罗伯特·塞奇威克写了一系列关于这个主题的通俗易懂的书（《算法》[SW11]、
《算法分析导论》[SF13]，
  等等）。 我们建议你将其中一本书加到收藏中， 最重要的是一定要读一读。

- 如果想比塞奇威克所讲更进一步，
  那就读一读高德纳那套权威的《计算机程序设计艺术》，
  该书在一个更宽的范畴分析了算法。
...
在[常识判断](#常识判断)一节里， 我们宣称二分法是 $O(\lg n)$ 的，
你能证明这一点吗？

练习 30（参考答案见[练习的参考答案](#练习的参考答案)）

在图 7-1 中， 我们宣称 $O(\lg n)$ 和 $O(\log_{10} n)$
...
不幸的是， 软件开发最常见的隐喻是建筑施工。
伯特兰·迈耶的经典著作《面向对象软件构造》[Mey97] 使用了“软件构造”这个术语，
甚至笔者也曾在 21 世纪初为 IEEE Software 的软件构造专栏做编辑。[^ch08-7]

但是用“构造”一词作为指导性隐喻， 意味着以下步骤：
...
重构并不是一种特殊的、隆重的、
偶尔进行的活动——不是那种为了重新种植而把整个花园翻耕一遍的大动作。
重构是一项日复一日的工作， 需要以低风险的小步骤进行，
...
一些特性现在看来更为重要， 而某些“必须拥有”的特性可能并不重要。
...
重构代码——把功能移来移去并更新早期的决策——实际上是一种疼痛管理的练习。
让我们面对现实， 修改源码可能相当令人痛苦： 代码已经能工作了，
 保持原样或许更好。
...
重构始于 Smalltalk 社区。 当我们编写第一版图书时， 它刚刚开始收获更广泛的拥趸。
这可能要归功于重构领域第一本主要著作（《重构：改善既有代码的设计》[Fow19]，
现在已经是第二版了）的出版。

重构的核心是重新设计。 你或团队中的其他人设计的任何东西，
都可以根据新的事实、更深的理解、更改的需求等重新设计。 但是，
但如果你执意要把海量代码统统撕毁， 可能会发现，
自己所处的境地比开始时更加糟糕。
...
有时想使用一些测试数据， 而这可能意味着要在一个真正可控的数据库中工作。
...
然后， 我们必须考虑如何填充测试数据。
需求指明要一份“每周观看 10 个以上视频的人的名单”。 因此，
我们查看了数据库的 schema， 找了找可能有帮助的字段。
我们在“谁看了什么”这张表中发现了两个可能的字段： opened_video 和
completed_video。 要编写测试数据， 我们需要知道该使用哪个字段。
但我们不知道需求到底是什么意思， 业务联系人又联系不上了。 让我们小小作弊一下，
传入字段名（这样既能测试现有的字段， 以后有需要时也还能改）：
...
我们看到了 TDD 对以测试为起点开发的人的主要好处。 只要遵循 TDD 工作流程，
...
- 他们做了很多冗余的测试。 例如， 在第一次编写类之前，
  许多 TDD 的信徒会先编写一个失败的测试， 仅仅只是简单地引用一下类的名称。
...
- 他们的设计倾向于从底层开始， 然后逐步上升。
  （参见下方的
[自上而下与自下而上之争，以你应该用的方式去做](#自上而下与自下而上之争以你应该用的方式去做)
）。
...
当时敏捷运动的领军人物罗恩·杰弗里斯开始撰写一个博客系列，
记录了他尝试用测试驱动的方法开发数独求解程序的过程。[^ch08-11]
在写了五篇文章之后， 他改进了底层棋盘的呈现方式， 进行了多次重构，
直到对对象模型满意为止。 但之后他放弃了这个项目。
按顺序阅读这些博客文章是很有趣的，
可以看到一个聪明人是如何被让测试通过的喜悦套牢， 开始为琐事分心的。

#### 自上而下与自下而上之争，以你应该用的方式去做

在计算科学尚处稚嫩童年的年代， 有两种设计学派： 自上而下和自下而上。
...
我们在开始时并不知道自己在做什么。 自上而下学派认为可以提前表达整个需求，
...
我们喜欢将单元测试看作是在针对契约测试（参见
[话题 23：契约式设计](#23-契约式设计)）。
我们想要编写测试用例来确保指定的单元遵守了契约。 这样做能告诉我们两件事：
代码是否符合契约， 以及契约的实际含义是否与我们所想的一致。
...
假设我们有一个模块 A， 使用了 DataFeed 和 LinearRegression。
 那么就可以依次测试：
...
这种技巧是减少调试工作的好方法： 我们可以快速聚焦到模块 A 中问题可能的来源，
而不会浪费时间反复检查它的子组件。
...
临时测试发生在我们手动运行代码随手摆弄的时候。
可能只是简单地加一句 console.log()，
也可能是在调试器、IDE 或 REPL 环境中交互输入的一段代码。
...
还有一些代码不变式， 即数据经过某个函数之后， 状态的某一部分仍然保持为真。
 例如，
如果对列表进行排序， 结果将具有与原始列表相同的元素数量——长度就是一个不变式。

一旦实现了契约和不变式（我们将把它们放在一起并称为特性），
就可以使用特性来自动化我们的测试。 这种技术被称为基于特性的测试。
...
提供了一门用来描述所要生成数据的小语言。 语言是基于对 hypothesis.strategies
...
测试函数将运行多次。 每次都会传入一个不同的整数。 如果我们这样写：
...
就能得到最多 100 个元素的正整数列表。

由于本书并不是某个特定框架的教程， 所以我们将跳过一些很酷的细节，
来看看一个真实的例子。
...
其中第一个元素是 “ok” 或 “not available”， 其余元素是物品和数量。
我们也写了一些测试， 并且它们通过了：
...
我们知道的一件事是， 库存不会在我们的交易中凭空出现或消失。 这意味着，
如果从仓库中取出一些物品， 取出的数量加上当前在仓库中的数量，
应该与最初仓库中的数量相同。 在接下来的测试中， 我们从 “shoes” 和 “hats”
...
测试在 warehouse.take_from_stock 处引爆： 我们试图从仓库中移除三顶帽子，
但仓库里只剩两顶了。
...
测试发现了一个 Bug， 但与库存水平调整无关。 相反， Bug 出在 in_stock 函数中。
...
并且大小不一。 你的工作是写一些代码， 把尽可能多的板条箱装进一辆单层的运输卡车。
...
数以亿计的记录顷刻间被盗，
数十亿美元的损失和赔付——这些数字每年都在快速增长。 在绝大多数情况下，
...
外部攻击者会抓住我们留下的每一个漏洞来破坏系统。 特定的开发和部署环境，
会有自己围绕安全性的特定需求， 但是你应该始终牢记一些基本原则：
...
系统的攻击面，
指攻击者可以输入数据、提取数据或调用服务的所有访问点的总和。
这里有几个例子：

代码复杂性滋生攻击载体

代码的复杂性会让攻击面更大， 会留下更多产生意料之外的副作用的机会。
可以将复杂的代码视作元凶， 是它使攻击面脆弱多孔、易受感染。 再者，
代码越简单越好。 更少的代码意味着更少的 Bug， 更少机会出现严重安全漏洞。
更简单、更紧凑、复杂度更小的代码更容易推理， 也更容易发现潜在弱点。

输入数据是一种攻击载体

永远不要信任来自外部实体的数据，
在将其传递到数据库、呈现视图或其他处理过程之前， 一定要对其进行消毒。[^ch08-15]
一些语言可以在这方面提供帮助。 例如在 Ruby 中，
包含外部输入的变量被认为是受污染的， 对其执行的操作会受到限制。 例如，
这段代码明显使用了 wc 程序来统计一个文件中有多少字符，
该文件名是在运行时提供的：
...
因此如不加任何处理和限制， 至少会立刻留下一个拒绝服务（DoS）攻击的机会。
最近多起被高度曝光的数据泄露事件，
是由于开发者不小心将数据放入无须身份认证、公开可读的云存储中造成的。

经过身份认证的服务成为攻击载体

将授权用户的数量保持在最低限度。 要淘汰不用的、陈旧或过时的用户和服务。
我们发现， 许多联网设备带有简单的默认密码，
...
它甚至会提示“该密码已被其他用户使用”这样的信息。 不要泄露信息，
...
不要自动获取类似 root 或 Administrator 这样的最高级别权限。
如果真需要这么高级别的权限， 那就去申请， 获得权限后只做最少量的工作，
然后迅速放弃权限， 这样可以降低风险。 这一原则可以追溯到 20 世纪 70 年代初期：
...
这个提示关乎每一台联网设备，
包括电话、汽车、家用电器、个人笔记本电脑、开发人员机器、构建机器、
生产服务器和云映像，
...
严格的密码策略实际上会降低安全性。 下面针对一些非常糟糕的想法， 给出 NIST
的一些建议：[^ch08-17]
...
我们应该鼓励长的随机密码， 因为它有更高程度的熵。 人为的限制降低了信息熵，
...
破解者是如何使用彩虹表之类的东西的， 为什么不应该使用 MD5 或 SHA1，
以及许多其他问题。
 即使一切都做得正确， 到头来你仍然有责任根据那些新的立法和法律义务，
 保留数据并维持数据的安全。

或者你可以采取务实的方法， 采用第三方提供的认证服务， 让别人去操心。
...
> 图 7-2：斯特鲁普效应演示面板。
> 面板由一组颜色名称的文字构成，每个名称以与其字义不同的颜色或灰度显示。
> 先大声读出每个词，再大声说出每个字的颜色——后者明显困难得多，
> 因为大脑阅读文字的速度远快于识别颜色。
...
这个自定义类型。
 不知道你怎么样， 反正我们在处理百分比时， 从来搞不清该值应该是 0 到 100 之间，
还是 0.0 到 1.0 之间。 使用一个专门类型， 可以把函数的预期文档化。

- 我们有一个模块， 是用斐波那契数列来做一些有趣的事情。 其中之一是计算序列中的第
...
有一个悠久的传统， 项目和项目团队都该有个朦胧而“聪明”的名字——宝可梦，
...
大多数的计算机入门文章都会告诫你， 永远不要使用单个字母的变量， 如 i、j、k。
[^ch08-21]
我们认为在某种程度上这并不正确。

事实上， 这取决于特定编程语言或环境所处的文化氛围。 在 C 语言中， i、j、k
通常用作循环变量， s 用于字符串， 等等。 如果在这种环境中编写程序，
那么你就会习以为常， 违反这种惯例反而显得突兀（也就说明不对）。 与此相对，
...
如果你不注意随时更新名字， 很快就会陷入比毫无提示的名字更糟糕的噩梦：
误导性的名字。 你是否曾经遇到过有人解释代码中的不一致性， 比如“名为 getData
的例程实际上会将数据写入归档文件”？

正如我们在[软件的熵](#3-软件的熵)中所讨论的， 当你发现一个问题时，
要立即修复它。 当你看到一个名称不再表达其意图， 或是具有误导性，
抑或是令人困惑时， 请修复它。 你已经有了完整的回归测试，
因此能发现任何漏改的位置。

> [!TIP]
> **提示 74 好好取名；需要时更名**

如果由于某种原因， 无法改变现在这个错误的命名， 那么说明这里有一个更大的问题：
 违背了
ETC 原则（参见[优秀设计的精髓](#8-优秀设计的精髓)）。
那么先修复这个更大的问题， 再更换有问题的名字。 让更名变得容易， 并且经常去做。

否则， 你将不得不向团队中的新成员解释， getData 实际上会将数据写入文件，
而且必须板着脸不笑出来。
...
[^ch08-11]: 网址参见链接列表 7.3 条目。“特别感谢”罗恩让我们使用这个故事。

[^ch08-12]: 网址参见链接列表 7.4 条目。
...
[^ch08-15]: 还记得我们的好朋友，小东西 Bobby
    Tables（网址见链接列表 7.5 条目）吗？一边回忆一边看看这个网站（网址见链接列表 7.6 条目），那里列出了一系列将数据传去做数据库查询前的消毒方法。
...
[^ch08-17]: NIST Special Publication 800-63B: Digital Identity Guidelines:
    ...
    Management，在网上能免费浏览（网址见链接列表 7.7 条目）。
...
但几乎所有的“敏捷”项目， 都是从讨论要采用哪些流程和工具开始的。 但是，
无论“敏捷”经过了怎样的深思熟虑， 包含了多少“最佳实践”，
都没有什么能取代思考。 你不需要任何特定的过程或工具，
...
它们被埋在层层假设、误解和办公室政治之下。 更糟糕的是， 需求通常根本不存在。
...
将其转化为设计文档， 然后变成流程图和伪代码， 最后写成代码。 这还没完，
在代码输入电脑之前， 我们还需要花时间在办公桌前检查一遍。

这样做会花很多钱。 这样的成本意味着，
人们只有在知道自己真正想要什么的时候才会尝试自动化。 由于早期的机器相当受限，
所解决问题的范围也受到了限制： 实际上， 你最好在开始之前就理解整个问题。
...
对当下面临的问题做出回应。 需求可能是对现有系统的变更，
也可能是需要某些新东西。 需求有时用业务术语表达， 有时用的是技术术语。

新手开发人员经常犯的错误， 是把这种对需求的声明照单全收， 然后照着实现。
...
这就是我们所做的。 当某些事情看起来很简单时， 我们却会去寻找那些边缘情况，
并不厌其烦地追问。

很可能客户已经想到了其中的一些问题， 并假定实现会以某种方式处理。
问这类问题只是为了把信息明确下来。 但有些问题可能客户之前并没有考虑到。
这正是事情变得有趣的地方， 也是优秀开发者得以脱颖而出的地方。
> **你**：我们想知道这个 50 美元的总数，包括了通常应收取的运费吗？
>
> **客户**：当然。总数指他们本应支付的金额。
>
> **你**：很好，消费者理解起来并不难，我自己都想去下一单了。
> 不过，我觉得有些不太正派的消费者会钻这个系统的空子。
>
> **客户**：会怎么做？
>
> **你**：这样，假设他们买了一本 25
...
这就开启了探索过程。 你的角色是解释客户所说的话， 并向他们反馈其中的含义。
...
在这种情况下， 务实的程序员借助“你是不是这个意思”这样的客户访谈来得到反馈。
我们制造出展示模型和产品原型， 并让客户先试用。 理想情况下，
我们生产的东西足够灵活， 经得起在与客户讨论过程中的修改；
当客户反馈“这不是我的意思”时， 我们就用这一句来回应——“是不是更像这样”。
...
那么开发者可能会在应用程序每次访问该数据的地方编写一个显式的检查。 但是，
如果声明是“只有授权用户才能访问员工记录”，
那么开发者可能会设计并实现某种访问控制系统。 当策略改变时（往往会变），
...
他们的耳朵与控制滑块和旋钮的指尖之间， 形成了一种自然的反馈回路。 然而，
新混音台的界面并没有放大这些技能。 相反， 它强迫用户在键盘上打字或点击鼠标。
它功能齐全， 却以一种陌生而怪异的方式呈现。 工程师需要的功能，
有时隐藏在晦涩的名称后面， 或者要通过一些并不直观的基本操作的组合来实现。

这个例子还阐明了我们的信念， 即成功的工具用起来让人觉得称手。
成功的需求采集也会考虑这一点。
 这也就是为什么我们用原型或曳光弹来获取早期反馈时，
...
但这并不意味着， 你可以不记录对客户需求的理解就蒙混过关。 这只是意味着，
...
这些详尽的文档， 是在客户最初两分钟需求说明的基础上扩展而来， 最终产出的杰作，
有一英寸厚， 充满图片和表格。 它事无巨细地一一指定下来，
几乎没有给实现留下任何含糊的余地。 如果有足够强大的工具，
这份文档实际上可以直接转换为最终的程序。

创建这些文档是一个错误， 原因有两个。 首先， 正如我们所讨论的，
客户并不确切知道自己想要什么。 所以， 当我们需要把他们所说的东西，
扩展成近乎法律文件的东西时， 就好像是在流沙上建造一座极其复杂的城堡。
...
而客户的动机只是解决一个高阶的、有些模糊的问题。 需求文档是为开发人员编写的，
...
这并不是贬低客户， 但给他们一份庞大的技术文档，
就像是给普通开发者一份荷马时代希腊语的《伊利亚特》，
并让他们据此编写出一个视频游戏。

#### 需求文档是为计划准备的

因此， 对于那些厚得吓人的大部头需求文档， 我们并不信任。 然而，
我们知道需求必须写下来， 因为团队中的开发人员需要知道即将做什么。

应该用怎样的形式？ 我们喜欢那些可以写在真实（或虚拟）索引卡上的描述。
这些简短的描述通常被称为用户故事。 它们从使用某功能的用户角度，
描述了应用程序的一小部分应该做什么。

当以这种方式编写需求文档时， 需求可以贴在一块板子上并四处移动，
以展示其状态和优先级。

你可能认为， 单张索引卡无法囊括实现应用程序组件所需的信息。 你说的没错，
但这正是目的之一——让需求保持简短， 正是为了鼓励开发人员去澄清问题。
这样可以在编写每段代码之前以及编写期间，
强化客户和程序员之间的反馈过程。
...
最简单而又最能准确反映业务需求的语句是最好的。
这并不意味着可以模棱两可——必须将底层语义中的不变式作为需求紧紧抓住，
...
许多项目失败，
都可以归咎于范围不断蔓延——也称为功能膨胀、特性泛滥或需求蠕变。
这是“沸水青蛙综合征”的一个方面，
参见[话题 4：石头做的汤和煮熟的青蛙](#4-石头做的汤和煮熟的青蛙)。
我们可以做些什么来防止需求蔓延呢？

答案（又一次）是反馈。 如果你在与客户一起工作时， 一直通过持续反馈来进行迭代，
那么客户对“再多一个特性”所产生的影响， 将有切身体会。
他们将看到另一张故事卡出现在板上， 继而帮你挑出一张卡舍弃，
以便为下次迭代腾出空间。 反馈是双向的。

#### 维护一张术语表

一旦开始讨论需求， 用户和领域专家就会使用对各自有特定意义的行话。
例如， “客户”和“消费者”可能有区别， 在系统中随意混用这两个词就不合适。

创建并维护一张项目术语表， 在上面记录项目中所有特定术语和词汇的定义。
项目所有的参与者， 包括最终用户和支持人员， 都应该使用同一张术语表来确保一致性。
...
下列各项的真正需求到底是什么？
找到使现在的陈述不够通用的部分， 并（如有可能）加以重述。

1. 响应时间必须少于 500ms。
2. 模态窗口需要有一个灰色的背景。
3. 应用程序将由许多前端进程和后端服务器构成。
4. 如果用户在数字框中输入了非数字字符，系统将闪烁输入框的背景且不接受输入。
5. 这个嵌入式应用程序的代码和数据必须在 32Mb 内。

#### 挑战

- 你能使用你正在编写的软件吗？
  在自己还不能使用软件的情况下，还可能对需求有良好的感觉吗？
...
现在遍历这份清单， 逐条解释为什么不能选这条路。 你确定吗？ 你能证明吗？

想想特洛伊木马——这是一个棘手问题的奇特解决方案。
你怎样才能在不被发现的情况下让军队进入一个有城墙的城市？ 可以打赌，
“从大门进去”最初肯定会被认为是自杀。

对约束进行分类和排序。 木工开始一个项目时， 会先切出最长的木块，
...
在日常工作中， 将什么行得通什么行不通反馈给大脑， 是喂养大脑的最好方法。
我们介绍过一个好方法， 就是记录工程日记（见[话题 22：工程日记](#22-工程日记)）。
...
必须设计一个全新的系统， 与过去（通常没有文档记录）的行为完全匹配。
...
各种研究都对这一观点提供了强有力的支持。
我们已经无数次亲眼目睹了这一点——例如， 团队中没有人互相交谈，
导致形成孤立的“烟囱”式系统。 或者一个团队被一分为二，
形成客户端/服务器或前端/后端部门。

这条定律的反向运用也有研究支持： 你可以有意地按照代码所期待的样子来组织团队。
例如， 地理上分散的团队更倾向于模块化、分布式的软件。

但最重要的是， 包含用户的开发团队所开发出的软件，
会清晰地反映出这种参与， 即使团队并不刻意去做。
...
结对编程有很多好处。
 不同的人有不同的背景和经验， 解决问题的技巧和方法也各不相同，
对任何特定的问题有不同的关注点。
充当打字员的开发者必须专注于语法和编码风格的底层细节，
而另一个人则可以自由地从更高的层面思考问题。 虽然这听起来像是一个小小的区别，
但请记住， 我们人类的大脑带宽有限。
光是敲入那些编译器勉强接受的深奥单词和符号，
就已占用了我们相当大的处理能力。 在执行任务的过程中，
有另一个开发人员的完整大脑可用， 可支配的脑力就更多了。

第二个人带来的同伴压力， 有助于克服脆弱的瞬间， 以及避免把变量命名为 foo
这样的坏习惯。 当有人盯着的时候， 也不太可能去走那些让你事后尴尬的捷径，
这也会让软件质量得到提高。

#### 群体编程

如果两个脑袋比一个要好， 那么让一群不同的人同时处理一个问题， 外加一个打字员，
可不可以呢？

尽管名为“群体编程”， 但这个群体并非乌合之众。 作为结对编程的扩展，
它不只涉及两个开发人员。 支持者声称， 用群体方式解决棘手问题效果很好。
群体可以很容易地将通常不被认为是开发团队一部分的人囊括进来，
比如用户、项目赞助者和测试人员。 事实上， 在我们的第一个“不可能完成的”项目中，
由一个人打字， 而另一个人与业务专家讨论这个问题，
是一个常见的场景——最后这样的组合成为一个三人的小团体。

你可以把群体编程想象成围绕现场编码展开的紧密协作。

#### 我该做什么

如果你目前是一个人独立编程， 可以尝试一下结对编程。 至少预留两周的时间，
每次持续几个小时， 因为一开始会感觉很奇怪。 在需要集思广益、想出新点子，
或诊断出棘手问题的时候， 不妨尝试做一次群体编程。

如果你已经在做结对或群体编程， 是和谁一起做？ 仅仅是开发者，
还是允许用户、测试人员、赞助者等周边团队的成员参与？

与所有协作事务一样， 你在人这方面需要投入的关注， 要和技术方面一样多。
这里有一些上手的小技巧：

- 打造代码，而非打造自我。
  这与谁最聪明无关；我们都有许多闪光的瞬间，也有糟糕的时刻。
...
那么不妨尝试一些不同的形式。 但是， 不要天真地贸然上手：
每种开发风格都有规则、建议和指导方针。 例如， 在群体编程中， 每隔 5-10
分钟就需要换一次打字员。

做一点阅读和研究， 从教科书和经验报告中， 了解一下可能获得的优势和会遇到的问题。
不妨从一个简单的练习开始， 而不是直接跳到最难的产品代码。
...
敏捷是一个形容词： 它描述的是你做事的方式。 你可以成为一名敏捷的开发人员。
你可以加入一个采用敏捷实践的团队， 一个对变化和挫折做出敏捷反应的团队。
敏捷指的是你的风格， 而不是你这个人。

> [!TIP]
> **提示 83 敏捷不是一个名词；敏捷关乎你如何做事**
...
让敏捷开箱即用。 他们想要的这个东西， 恰恰是许多咨询公司和咨询师乐于推销的。
结果这些公司采用了更多层的管理、更正式的报告、
更专业的开发人员和更花哨的岗位头衔，
...
那么这样的人， 显然并不认同我们和其他宣言作者所重视的东西。

任何向你推销“开箱即用方案”的人都没有读过这份介绍性声明。 这些价值观，
是由不断探寻更好软件开发方式的过程所激发和揭示的。 这不是一份静态的文档，
它是对生产过程的建议。
...
对开始之后遇到的未知情况做出的响应。 奔跑中的羚羊不会沿直线前进，
体操运动员一秒钟要纠正数百个错误，
...
这些决策总是与环境相关：
它们取决于你是谁、团队的性质、你的应用程序、你用的工具、你的公司、
你的客户以及外部世界；
因素很多， 有重要的也有不重要的。 任何固定的、静态的计划，
都无法应对这种不确定性。
...
通过在非常底层的层面（对变量的命名）上应用反馈循环，
我们实际上改进了整个系统的设计， 减少了这部分代码与处理账户的代码之间的耦合。

反馈循环也适用于项目的最高层级。 我们最成功的一些工作成果，
是在开始处理客户需求时偶然产生的。 当我们迈出一小步时，
就意识到将要做的事情并不是必须做的， 最好的解决方案甚至不涉及软件。

此循环不仅仅能用于单个项目。 团队应该用它来回顾流程及其效果。
不持续对流程做实验的团队， 不是敏捷团队。

#### 还可以用来驱动设计

在[话题 8：优秀设计的精髓](#8-优秀设计的精髓)中， 我们曾断言，
衡量设计的标准， 是变更设计结果的容易程度：
优秀的设计比糟糕的设计更容易变更。

关于敏捷的讨论解释了为什么会这样。

你做出了一个改变， 后来发现自己并不喜欢。 根据秘诀中的第三步，
我们必须能够修复自己弄坏的东西。 为了使反馈循环更有效， 修复必须尽可能地简单。
如果不够简单， 我们就只会耸耸肩， 而不去修它。
我们在[话题 3：软件的熵](#3-软件的熵)中讨论了这种效应。 要让整个工作敏捷起来，
就需要践行优秀的设计， 因为优秀的设计使事情容易改变。 如果它很容易改变，
就可以在每个层面做调整， 不会有任何犹豫。
...
简单的反馈循环并不仅仅适用于软件。 回想一下最近在软件之外做的决定。
如果事与愿违， 假设一切都可以回退重来， 你会有什么不同的做法？
...
一旦项目不只你一个人， 就需要建立一些基本规则， 并将项目的各个部分委派出去。
在[务实的团队](#49-务实的团队)中，
...
当然， 如果你连持续可靠地交付软件都做不到， 其他一切也就无从谈起了。 这一前提，
引出了由“魔法三连”——版本控制、
测试和自动化——组成的[务实的入门套件](#51-务实的入门套件)。

然而， 成功与否最终取决于旁观者（项目的赞助者）的眼光。 成功的感觉才是最重要的，
...
甚至在 1985 年， 关于养猫的笑话就已经很老了。 到了世纪之交第一版问世时，
这已经是个相当古老的笑话。 然而它却历久弥新， 因为它揭示了真相。
程序员有点像猫：聪明、意志坚强、固执己见、独立， 并且常常在网上受到狂热追捧。

到目前为止， 我们已经在这本书中看到，
务实的技巧能帮助一个人成为更好的程序员。 这些方法是否也适用于团队，
...
务实的团队很小， 充其量也就 10-12 人。 成员很少变动。 每个人都很了解彼此，
相互信任， 互相依赖。

> [!TIP]
> **提示 84 维持小而稳定的团队**

在本部分中， 我们将简要地了解如何将务实的技巧应用到整个团队。
这些提示只是一个开始。 一旦你将一组务实的开发者置于一个赋能环境中工作，
...
水盆里那只毫无戒心的青蛙吗？ 它没有注意到环境在逐渐变化， 最终被煮熟了。
同样的情况也会发生在那些不警惕的人身上。 在项目开发最紧张的时候，
很难保持对整个环境的关注。
...
太多的团队忙于往外舀水， 而没有时间修补漏洞。 把这件事排入日程表并加以解决。
...
与代码的实际用户隔了两三层的程序员， 不太清楚自己工作所处的上下文，
也就无法做出明智的决定。

使用曳光弹时， 我们建议开发单独的特性， 无论它们最初是多么小、多么局限，
都要贯穿整个系统。 在团队中要实现这一点，
意味着团队需要具备所有这些技能：前端、UI/UX、服务器、DBA、QA， 等等，
而且这些技能彼此之间也需要相互协调、配合默契。 使用曳光弹方法，
...
当部署可以每次都以相同的方式自动、可靠地重复进行时， 为什么还要手工部署呢？

自动化是每个项目团队的基本组成部分。 确保团队具备构建所需工具的技能，
以便开发出所需的工具， 将项目开发和生产部署自动化。
...
原生岛民以前从未见过飞机， 也没见过像这些陌生来客那样的人。
这些陌生人给他们的家乡带来了难以置信的物质财富。 他们操纵着机械大鸟，
在被称为“跑道”的地方整日起降， 并用这些东西回报当地人允许他们使用自己的土地。
 陌生人提到这可能与战争和冲突有关。
直到有一天， 一切都结束了， 他们离开了， 带走了他们那些奇异的财富。

岛民们急于重现好运，
他们用当地的材料建造了机场、控制塔和设备的仿制品：使用的是葡萄藤、椰子壳、
棕榈叶等。
...
例如， 我们亲眼见过一些团队， 宣称在使用 Scrum。 但稍加盘问就会发现，
...
你或团队掉进这个陷阱了吗？ 问问自己， 为什么要使用这种特定的开发方法？
...
实际上， 你要做的是汲取每种特定方法中最好的部分， 并加以调整后使用。
...
> 图
> 9-1：交付时间与交付方式的关系。
> 纵轴为交付时间（年、周、小时），横轴从左到右依次为：
> 数月/年为期的迭代（瀑布）、固定短周期迭代（Scrum）、持续交付。
> 曲线随交付方式的演进而持续下降。

如果交付周期是几年， 试着把周期缩短到几个月。 如果是几个月， 就减少到几周。
如果是一个四周的冲刺， 那么试一下两周。 如果是两周的冲刺， 试试一周。
然后到每天。 最后是按需交付。 请注意，
能够按需交付并不意味着你必须每天每分钟都交付。 只有当这样做在业务上有意义时，
才有必要在用户需要时按需交付。
...
我们将在下一话题[务实的入门套件](#51-务实的入门套件)中讨论。
你会在版本控制系统的主干上进行开发， 而不是在分支上进行，
并且使用诸如特性切换之类的技术， 选择性地向用户推出试验性特性。
...
一个按照指令清单操作的人可能会让发动机熄火， 但自动启动器不会。
...
但无法保证可重复性， 尤其是当操作过程的某些部分允许不同的人各自解读时。

在我们写完第一版的《程序员修炼之道》之后， 希望创作更多的书来帮助团队开发软件。
我们认为， 应该从头开始：抛开方法、语言或技术栈，
每个团队需要的最基本、最重要的元素是什么？ 因此， 务实的入门套件这个想法诞生了，
它涵盖了三个关键且相互关联的主题：
...
另外， 知道测试全部通过， 能让你对代码已经“完成”这一点充满信心。

> [!TIP]
> **提示 91 直到所有的测试都已运行，编码才算完成**

自动构建过程要运行所有可用的测试。 要在尽量真实的环境中进行测试， 换句话说，
测试环境应该与生产环境紧密匹配。 任何缝隙都是 Bug 滋生的地方。

构建可能包括几种主要的软件测试类型：单元测试、集成测试、确认和验证，
以及性能测试。

这个列表远不算完整， 一些特殊的项目还需要各种其他类型的测试。
但这是一个很好的起点。
...
以及它们与开发人员所用测试数据的差异（[话题 20：调试](#20-调试)
中关于笔刷的故事就是一例）。
...
我们的测试套件可以看作是一个精心设计的安全系统， 用来在出现 Bug 时发出警报。
要测试一套安全系统， 还有什么比费力闯入更好的办法呢？
...
并验证测试是否能够捕获到。
 在更高的层次上， 可以使用像 Netflix 的 Chaos Monkey[^ch10-ch9-8]
...
一旦你确信测试是正确的， 并且它们确实在承担寻找 Bug 的工作，
那么如何知道是否已经对代码库进行了足够彻底的测试？

简单的回答是“无法知道”， 永远也不会知道。
你可以试试能在测试期间监视代码覆盖率的分析工具， 并跟踪哪些代码行已经执行，
哪些没有执行。
 这些工具帮助你大致了解测试有多全面， 但是不要期望 100% 的覆盖率。
[^ch10-ch9-9]
...
[话题 42：基于特性测试](#42-基于特性测试)对此有详细的讨论。

#### 绷紧渔网

最后， 我们想要揭示测试中最重要的概念。 它显而易见，
几乎每一本教科书都会告诫我们要这么做； 但由于某些原因， 大多数项目仍然没有做到。
...
一个 Bug 一旦被人类测试员发现， 就应该是它最后一次被人类发现。
...
正如我们在本部分开始时所说的， 现代开发依赖于脚本化的自动化流程。 无论你是使用
...
这些说明足足写满了好几页——全是点击这里、滚动那里、拖动这个、双击那个，
以及再做一遍。
...
因此你也可以沿时间线回溯检查构建/发布过程的变更（“之前明明可以用的……”）。
...
- 你能自动测试你的项目吗？ 许多团队无奈地回答“做不到”。 为什么？
  对可接受的结果做出定义， 是不是太难了？
  向赞助商证明项目已经“完成”是不是也会因此变得更加困难？

- 对应用程序中独立于 GUI 的那部分逻辑做测试， 是否太难了？ 这说明了和 GUI
  有关的什么问题？ 是耦合问题吗？
...
不要为了数据就去深挖他们， 不要数他们的眼球， 不要掏空他们的钱包。
...
- 根据期望严格分析用户需求。 在许多项目中， 我们已经发现，
  所陈述的“需求”往往只是对技术能实现什么的猜测——其实是一个业余的实现计划，
  只是伪装成了需求文档。
 如果你能证明某种方法能让项目更接近目标， 那么就不要害怕，
  大胆提出改变需求的建议。

- 随着项目的进展， 继续考虑这些期望。

我们发现， 随着对该领域了解的加深， 我们能够更好地对更多事情提出建议，
从而解决背后的业务问题。 我们坚信， 那些接触过组织的许多不同方面的开发人员，
...
如果你想取悦客户， 就和他们建立起某种关系， 这样才能积极地帮助他们解决问题。
...
或者不愿意为共同的基础元素工作。
 项目可能会四分五裂， 退化成一个个孤立的诸侯小国。
对于该偏向自己的代码还是同事的代码， 你会产生偏见。
...
虽然代码的所有权必须有所归属， 但不必归个人所有。 事实上，
肯特·贝克的极限编程[^ch10-ch9-11]建议共享代码所有权（但这也需要额外的实践，
比如结对编程， 以防匿名带来的风险）。

我们想看到你对所有权引以为豪——“这是我写的， 我与我的作品同在”。
你的签名应当被视为质量的标志。 人们应该在一段代码上看到你的名字，
并对它是可靠的、编写良好的、经过测试的、文档齐全的充满期待。
...
许多非嵌入式系统同样有好坏两面。 社交媒体可以促进和平改革，
也能煽动丑陋的仇恨。 大数据可以让购物变得更容易，
也可以摧毁你原以为自己还拥有的隐私。 银行系统做出的贷款决策，
足以改变人们的生活。 几乎任何系统都可以用来窥探用户。

我们已经隐约看到乌托邦未来的可能性， 也见过不少例子，
 意外后果如同反乌托邦的噩梦。
...
有一些创造性的想法， 开始在道德行为的界限上打擦边球， 如果你参与了这类项目，
就和出资者一样负有责任。

下面这条规则的正确性， 并不取决于你与“为虐”之间隔了几度分隔：
...
在面向对象语言中，
诸如多重继承、异常、操作符重载和重写父类方法（通过子类化）等特性，
都有充足的机会以不明显的方式增加耦合。 还存在一种耦合，
源于类将代码与数据耦合在一起。
这通常是一件好事（如果是好的耦合，我们称之为内聚）。
...
在定义小时和分钟时， 最好能考虑到， 小时只能取 00 到 23，
而分钟只能取 00 到 59：
...
- 该目录可能已被另一个进程删除，你可能没有权限读取，
  驱动器可能没有挂载进来……都不难想象。
- 我们没有显式指定 a 和 b 的类型。操作符重载可能定义了 +、= 或
  !=，这样可能会产生意外的行为。而且 a 和 b
  还可能是同一个变量的别名，这样第二个赋值会覆盖第一次赋值存入的值。
另外，如果程序是并发的，并且编写得很糟糕，那么做加法时
  a 的值可能已经被更新。
- 在非欧几里得几何中，三角形内角之和不会等于
  $180^{\circ}$——想象一个画在球面上的三角形。
- 闰秒则会让一分钟有 61 秒或 62 秒。
...
被指向的内存届时很可能已经被重新分配， 用于其他目的。 为防止这类悬空引用，
...
也就是说， 处理一个块的结果会影响其他块，
比如在机器视觉应用程序或复杂的三维图像扭转变换中， 你可能会考虑黑板系统。
...
键值对列表通常假设键是唯一的， 而哈希库的实现，
要么依赖哈希表自身的行为来处理， 要么会在重复键出现时显式抛出错误。
...
根据国际电信联盟的规范， 国际电话格式以 + 号开头。 在一些场合也会使用 *
...
> 1:100,000，其余为
> 1:1,000），纵轴为时间（ms）。
> 冒泡排序增长最快，选择排序与插入排序次之，快排最慢。

### 答案 29（来自练习 29）

有几种方法可以达到这个目的。 其中一种可以彻底解决问题： 如果数组只有一个元素，
我们就不会进入循环。 每一次额外的迭代， 都使我们能搜索的数组的大小加倍。
