...
假设你想提出一个建议： 用一个基于网页的系统让最终用户提交错误报告。 根据听众的不同，
...
这提供了一种非侵入性的通信方式——甚至可以跨越多个场所——为所说的每件事都留下永久记录。
...
- 将变更限制在局部后，开发时间和测试时间都会减少。编写相对较小的、独立自主的组件比编写完整一大块代码要容易。简单的组件可以在设计、编码、单元测试之后置之不理——只需新增代码时，没必要一直改动已有的代码。
- 正交的方法同时促进了重用。如果组件有一个职责定义清晰的规范，就能以原作者预想不到的方式与新组件组合使用。系统耦合越松散，重新配置和改造系统就越容易。
- 组合正交组件能获得相当微妙的生产力提升。假设一个组件能做 M
  件独特的事情，另一个能做 N 件。如果它们是正交的，组合起来就能做 $M \times N$
  件事。如果两个组件不正交，有重叠之处，结果就少一些。在正交组件的组合过程中，单个单元的价值被放大了。
...
- 你不会被特定的供应商、产品或平台紧紧束缚，因为第三方组件的接口仅占整个开发中相对很小的一部分。
...
代码中需要包含所有的错误检查、结构、文档、自检查——这些任何生产代码都应具备的东西。
...
因此， RSpec
和路由代码是嵌入在运行代码之中的：它们对你的编码词汇表做了真正的扩展。 Cucumber 和
...
- 如果决定采用小语言作为一种更接近问题域的编程方式，那么你就要做好准备多花一些力气来加以实现。将为一个项目开发的框架复用到别的项目，你能找到对应的方法吗？
...
- 命令补全。 大多数 Shell 会补全命令和文件的名称——输入头几个字符， 按一下 tab 键，
...
然而这还不算完！ 有些人倒是在用版本控制，
但把仓库（保存着所有的修改历史）指向一个网络地址或是放在云存储中——他们深信找到了两者兼顾的最佳方案：有个公共仓库可以随时随地往里面放东西，
...
获取数据集的副本， 并用这份数据测试本地运行的应用程序副本， 确保其在本地也能崩溃。
...
同样， 如果 Bug 是损坏的数据造成的结果， 而数据在引爆程序之前经过了好几层的传播，
可以试试给函数加上更完备的参数检查，
...
有些人发誓说决不会把修边机放进工具箱里， 因为它吵吵闹闹、脏乱不堪， 只有一股蛮力；
...
对于做一些实验性的东西来说， 这种放大效果至关重要。 花 30 分钟尝试一个疯狂的想法， 比花 5 个小时要好得多。
花一天时间对项目的重要组件进行自动化是可以接受的； 但要花上一周时间就未必了。
在《程序设计实践》[Kp99]一书中， 柯林汉和派克用 5 种不同的语言构建了相同的程序——Perl 版是最短的（17 行，
而 C 语言版本则要 150 行）。 使用 Perl， 你可以处理文本、与程序交互、通过网络通信、驱动 Web
...
然后接收一个目录， 把其中每个 .yaml 文件转换为相应的 .json 文件（因此 database.yaml
...
在我们暂时搁置某件事的时候留个提醒， 记录一些疯狂的想法， 有时只是涂鸦。[^ch04-10]
...
sqrt 将返回特殊值 NaN（Not a Number， 非数字）。 而你可能要到程序稍后， 尝试对 NaN
进行数学运算时， 才会看到令人惊讶的结果。
...
可能有一个网络错误或文件系统错误没有被捕获， 然后得到了空的或损坏的数据。 也可能是
...
如何保证在异常抛出之前分配的所有内容都已被依次释放？ 答案在一定程度上取决于语言支持。
你通常有两个选择：
...
认知偏见往往使我们对悄然逼近工作的变化视而不见（参见[石头做的汤和煮熟的青蛙](#4-石头做的汤和煮熟的青蛙)）。
...
其中包含一组与语言无关的原则，并记录了一些常见的实现。 在这里，我们选用 JavaScript
的 RxJs 库。
...
和 stop 这两个方法， 而实现 Locatable 的类必须实现 getLocation 和 locationIsValid。
这意味着，我们之前的 Car 类定义，只有在包含所有这四个方法时才有效。

接口与协议之所以如此强大， 是因为我们可以将它们用作类型，
而实现了相应接口的任何类都将与该类型兼容。 如果 Car 和 Phone 都实现了 Locatable，
...
接下来，当我们写一个新的类来做一些持久化相关的事情时， 也可以把这个 mixin 加上去：
...
例如，我们可能正在为一台自动化椰林飘香鸡尾酒制造机编写软件。被告知的制作步骤如下：
...
它们通过发送消息进行通信。 每一个进程都与其他进程相互隔离，进程之间没有状态共享。
...
也能觉察到几个像 Fred 这样的人， 但你更需要对自己警醒——我们从不依赖巧合， 真的吗？
...
你最终依赖的， 是一些文档中未记载、因而未被考虑到的边界条件， 或是将错就错的结果。
...
整个庞大的代码库就会随着时间的推移而退化， 直到难以维持大量的 +1 和 -1 语句。 最终，
没有一处是正确的， 这个项目被废弃了。
...
- 不要成为历史的奴隶。 不要让现有的代码去支配未来的代码。 如果不再合适，
  ...
  这个决定可能会影响项目进度。 这里的假设是， 其影响小于不做更改所付出的代价。[^ch08-5]
...
则会花掉 33 s。 如果你很不幸地写出一个 $O(n^2)$ 的例程， 那就得坐下来耐心等上 100 s
...
那么就能知道代码运行大概需要多长时间。
如果这些数字依赖于外部因素（例如通宵批处理要处理的记录数量， 或是名单中的人名数量），
...
重构并不是一种特殊的、隆重的、偶尔进行的活动——不是那种为了重新种植而把整个花园翻耕一遍的大动作。
...
让我们面对现实， 修改源码可能相当令人痛苦： 代码已经能工作了， 保持原样或许更好。
...
有一个编程学派主张： 既然预先考虑测试有这么多好处， 为什么不干脆先把测试写出来呢？
...
假设我们有一个模块 A， 使用了 DataFeed 和 LinearRegression。 那么就可以依次测试：
...
还有一些代码不变式， 即数据经过某个函数之后， 状态的某一部分仍然保持为真。 例如，
...
现在可以用代码来表达。 大多数语言都有某种基于特性的测试框架。 这个例子是用 Python
...
不过， 将安全级别设置为 1 会把外部输入标记为受污染， 这意味着它不能用于危险的上下文中：
...
让你看到更大的图景。 当我们考虑一个变量或函数的作用时， 考虑的是它有什么特别之处，
...
> 图 7-2：斯特鲁普效应演示面板。
> 面板由一组颜色名称的文字构成，每个名称以与其字义不同的颜色或灰度显示。先大声读出每个词，再大声说出每个字的颜色——后者明显困难得多，因为大脑阅读文字的速度远快于识别颜色。
...
这个自定义类型。 不知道你怎么样， 反正我们在处理百分比时， 从来搞不清该值应该是 0 到 100 之间，
...
如果由于某种原因， 无法改变现在这个错误的命名， 那么说明这里有一个更大的问题： 违背了
...
成功的需求采集也会考虑这一点。 这也就是为什么我们用原型或曳光弹来获取早期反馈时，
...
- 你能使用你正在编写的软件吗？在自己还不能使用软件的情况下，还可能对需求有良好的感觉吗？
...
因为正是认识到这些， 才能找到答案。
这就是为什么有些谜题很能说明问题——我们太容易把潜在的解决方案排除在外。 举个例子，
...
- 仔细看看你今天卷入的难题。你能解决这个棘手的问题吗？一定要这样做吗？真的需要做吗？
...
结对编程有很多好处。 不同的人有不同的背景和经验， 解决问题的技巧和方法也各不相同，
对任何特定的问题有不同的关注点。
...
引出了由“魔法三连”——版本控制、测试和自动化——组成的[务实的入门套件](#51-务实的入门套件)。
...
岛民们急于重现好运，
他们用当地的材料建造了机场、控制塔和设备的仿制品：使用的是葡萄藤、椰子壳、棕榈叶等。
...
希望能吸引来真正管用的魔法。 但与最初发生在美拉尼西亚的货物崇拜一样[^ch10-ch9-5]，
用椰子壳制成的赝品是替代不了真机场的。
...
集成测试实际上只是对我们所描述的单元测试的扩展——你只是在测试整个子系统履行契约的情况。
...
- 根据期望严格分析用户需求。 在许多项目中， 我们已经发现，
  ...
  只是伪装成了需求文档。 如果你能证明某种方法能让项目更接近目标， 那么就不要害怕，
  大胆提出改变需求的建议。
...
或者不愿意为共同的基础元素工作。 项目可能会四分五裂， 退化成一个个孤立的诸侯小国。
...
我们已经隐约看到乌托邦未来的可能性， 也见过不少例子， 意外后果如同反乌托邦的噩梦。
...
Split2 可以分割从文件中读取的多行数据， 也可以分割由另一个例程生成或通过环境传入的数据。
...
- 我们没有显式指定 a 和 b 的类型。操作符重载可能定义了 +、= 或
  ...
  还可能是同一个变量的别名，这样第二个赋值会覆盖第一次赋值存入的值。另外，如果程序是并发的，并且编写得很糟糕，那么做加法时
  a 的值可能已经被更新。
