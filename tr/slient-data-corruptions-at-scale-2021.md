...
在大规模基础设施中，
我们已在集群的数十万台机器上运行了庞大的 silent error 测试库。
...
主要关注辐射造成的软错误或合成故障注入。
相比之下，我们观察到，
SDC 并不限于辐射与环境效应造成的、可用概率模型刻画的软错误。
设备特性同样可能导致 SDC，
而且这种错误可以大规模复现。
...
该错误率根据辐射引起的软错误率（SER）推算，
估计结果为 50,000 FIT
（Failure-In-Time；1 FIT 相当于每十亿设备小时发生一次故障）。
因此，研究者建议使用 ECC，
将 SRAM 的错误率降低 1,000 倍。
...
- **特殊功能单元：**
  ...
  特殊功能寄存器和栈架构。
- **main frame：**
  独立运行的复现程序必须具备适当的 main frame 和 function frame，
  否则就不完整。
  这些 frame 使代码可以执行。
...
需要注意的是，
运行该应用的所有机器上都没有任何日志或系统级健康信息
能表明存在这种故障模式。
我们发现过一些案例，
其中损坏影响了涉及非零操作数和非零结果的计算。
例如，
缺陷 CPU 执行了以下错误计算。
我们发现，
对于特定数据值，
