...
模型合成的可复用工具是组件级自修改的一个较窄的前身 [14]。每一次这样的修改本身就是动态组合的一个实例。
...
通过把效应提升为可逆的运行时模型、把系数提升为响应式的依赖解析机制，我们为动态可组合性获得了统一的形式基础，它语言无关，
适用于任何需要动态组合的软件架构。我们做出以下贡献：
...
当它发出的句柄不被该键的任何运算比较时，
$\simeq_k$ 可以把两个堆关联到相差一个句柄重命名的程度——这正是 CompCert
...
installed$_m(\gamma) \wedge k \in p_n$ 一致，
因为那里一个键只有一个可能的提供者。
...
本节以它们的全局形式从这些规则读出可组合性的两个维度——一个 fiber 的保证在其余 fiber
于其间做什么都成立——并补充只有整个系统才能被要求的东西：
...
而效应函数所受的见证条件正是定义 37 给出的那个条件，对迭代器按定义 51 所给出的方式理解，
...
它在 $u$ 处关闭，当 installed$_n^u$ 成立而 installed$_n^{u+1}$ 不成立，
而最终的片段未必如此关闭。
...
第二次查表是：
一个剥去名称以外一切的条目对规则不可见——正是这一点让定义 47 能在一个其恢复的状态里没有 fiber 之处使该
fiber 退休，并让引理 72 能移除被删除片段所做的注册。
...
1. step^{r} = L-Finish(n) 且 \theta_{n}^{r+1} = Active(-,
   \omega)；
...
本节剩余部分自底向上构建核心库。
5.1.1 节实现可逆效应——这是上下文被修改所经由的唯一原语；5.1.2 节在其上实现响应式系数；
5.1.3 节把二者组合进组件生命周期；5.1.4 节暴露构建于其上的上下文级操作。
...
> 为语言无关起见我们显式写出它：在急切调度（如 TypeScript promise）下，该调用是隐式的，
...
在受管运行时中，这采取程序化模块注册表的形式：已装载模块可从注册表驱逐，并在不再被引用后由垃圾回收回收；
...
模块实例由宿主回收器回收；在原生嵌入器（如 Wasmtime）丢弃它时被释放。
...
例如经由 JavaScript 的 Proxy 对象 [64] 或 Python 的描述符协议（__get__）
[65]。
...
被限制在其词法作用域内，并通过装箱恢复一等使用，装箱通过在类型中跟踪被捕获的能力来解除这一限制；Cordis 则相反，
...
Heunen 等人 [87]
在可逆设定下通过把 Hughes 的 arrow 改造为 *dagger arrow* 和 *inverse
arrow* 来建模副作用，捕获了串行化和可变存储等其操作存在逆的效应。
...
在预先固定的作用域内自动逆转效应，或从运行时通过在接口上拦截而累积的记录中回收资源。
...
代数效应处理器可以挂接在拆除时运行的终结器 [104]；事件溯源 [105] 通过追加补偿事件而非执行逆来撤回状态。
...
Angular 的分层注入器），但两者都不做响应式重新解析：当提供者在运行时被替换或移除时，
