...
are not clicked. Every day, China Mobile collects 5–8TB of phone
...
ahead). We have found that both the production and the
...
For each topic T that \(C_i\) subscribes to {
    ...
    compute \(P_T = \text{partitions available in all brokers under topic T}\)
    compute \(C_T = \text{all consumers in G that subscribe to topic T}\)
    sort \(P_T\) and \(C_T\)
    ...
    for each assigned partition p {
        ...
        let \(O_p = \text{the offset of partition p stored in the offset registry}\)
