...
> 未命中时从数据库获取并回填缓存；
> 写请求则先写数据库再删除缓存中的陈旧数据。

**查询缓存：** 我们依靠 memcache 来减轻数据库的读取负载。
特别是，
我们使用 memcache 作为*按需填充的旁路*缓存，
如图 1 所示。
当 Web 服务器需要数据时，
...
然后向 memcache 发送 delete 请求以使任何陈旧数据失效。
我们选择删除缓存数据而不是更新它，
因为删除是幂等的。
Memcache 不是数据的权威来源，
因此允许驱逐缓存数据。
...
陈旧写入和惊群效应。
陈旧写入发生在 Web 服务器在 memcache 中设置的值不反映应缓存的最新值时。
...
我们可以通过识别返回略微陈旧的数据是可接受的情况来进一步减少这个时间。
当一个键被删除时，
其值被转移到一个保存最近删除数据项的数据结构中，
在那里短暂存活后被清除。
get 请求可以返回租约令牌或标记为陈旧的数据。
能够使用陈旧数据继续前进的应用不需要等待从数据库获取最新值。
我们的经验表明，
由于缓存值往往是数据库的单调递增快照，
...
修改权威状态的 SQL 语句会被补充上事务提交后需要失效的 memcache 键 [7]。
...
则该数据项将在冷集群中无限期地不一致。
Memcached 的 delete 支持非零的 hold-off time，
在指定的 hold-off time 内拒绝 add 操作。
默认情况下，
...
考虑主区域中已完成修改数据库并试图使现在陈旧的数据失效的 Web 服务器。
在主区域内发送失效是安全的。
然而，
让 Web 服务器在副本区域使数据失效可能为时过早，
因为更改可能尚未传播到副本数据库。
随后从副本区域对数据的查询将与复制流竞争，
从而增加将陈旧数据设置到 memcache 中的概率。
历史上，
...
才应允许从副本数据库进行缓存填充。
否则，
后续请求可能导致副本的陈旧数据被获取和缓存。

我们采用 remote marker 机制来最小化读取陈旧数据的概率。
标记的存在表明本地副本数据库中的数据可能陈旧，
查询应重定向到主区域。
当 Web 服务器希望更新影响键 $k$ 的数据时，
该服务器 (1) 在该区域设置 remote marker $r_k$，
(2) 向主区域执行写入，
...
在这种情况下，
我们明确地用缓存未命中时的额外延迟换取读取陈旧数据概率的降低。

我们通过使用区域池来实现 remote marker。
请注意，
此机制可能在对同一键的并发修改期间暴露陈旧信息，
因为一个操作可能删除了另一个进行中操作应保留的 remote marker。
值得强调的是，
我们对 memcache 用于 remote marker 的使用在微妙的方式上不同于缓存结果。
作为缓存，
删除或驱逐键始终是安全的操作；
它可能增加数据库的负载，
但不会损害一致性。
相比之下，
remote marker 的存在有助于区分非主数据库是否持有陈旧数据。
在实践中，
我们发现 remote marker 的驱逐和并发修改的情况都很罕见。
...
如图所示，
所有页面请求的 56% 联系少于 20 台 memcached 服务器。
按量计算，
用户请求倾向于请求少量缓存数据。
然而，
这个分布有一个长尾。
