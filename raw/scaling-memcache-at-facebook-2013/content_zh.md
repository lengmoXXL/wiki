...
这些大小被求和并乘以一百万来近似工作集。
每日和每周工作集之间的差异表示流失量。
具有不同流失特征的数据项以一种不幸的方式交互：
仍然有价值的低流失率键在不再被访问的高流失率键之前被驱逐。
