...
另一些则不敏感。
Dynamic Frequency and Voltage Scaling（DFVS）使频率和电压以复杂的方式紧密关联，
...
- 让应用特定的机制承担一部分责任，应用“端到端论证”[20]——正确性往往最好在端点处检查，而不是在底层基础设施中检查。
- 系统支持高效的检查点，以便在计算失败后换一个核重启恢复。
...
- **可测试性设计**：使检测带有细微制造缺陷的核更容易，并把这些测试功能开放给终端用户（用于 scrub 在用机器）；
- **持续验证**：功能单元始终检查自己的结果；
