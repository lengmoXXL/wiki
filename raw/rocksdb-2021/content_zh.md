...
> 图 4。RocksDB 中四类校验和的作用范围。从上到下依次为：预写日志（WAL）校验和、SSTable 数据块校验和、SSTable 元数据块校验和，以及内存中的 MemTable 校验和。
...
我们认为既需要端到端校验和，
也需要交接校验和；这一主张仍与经典的端到端论证 [55] 一脉相承，
也类似于其他系统采用的策略：
[61]、ZFS [71]、
Linux [48] 和 [70]。
我们提出更早检测损坏，
