...
也改变了许多有状态服务的实现方式。
SSD 在读写两方面都能提供每秒数十万次输入/输出操作（IOPS），
比机械硬盘快数千倍，
还可以支持每秒数百 MB 的带宽。
不过，由于编程/擦除周期的次数有限，
高写带宽无法长期维持。
...
（iii）把 MemTable 的内容刷写到磁盘上的“有序字符串表”（Sorted String Table，
SSTable）数据文件；
...
| ---|---|---|---|---|---|---|
| 分层压实（leveled compaction）|16.07|9.8%|9.5%|0.99|1.7|1.84|
| 分级压实（tiered compaction）|4.8|94.4%|45.5%|1.03|3.39|4.80|
...
如图 1 中的虚线框所示。
分级压实（在 RocksDB 中称为通用压实（universal compaction））
