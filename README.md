...
- 行长限制：每行显示宽度不超过 80，中文字符按 2 宽度计（即 ASCII 字符计 1）。原则上一行只表达一个意思，按语义断行。断点不能落在两个相邻汉字之间（即不得把连续的中文词句从中间拆开），只能落在标点之后、空格或链接边界处；无标点可断时允许略超宽。图片引用行 `![alt](url)` 的 alt 须保持单行（可超出 80），便于 git diff 逐行查看。
